document.addEventListener("DOMContentLoaded", async function () {
  try {
    ...
    const employee_contracts_response_promise = fetch(
      "https://script.google.com/macros/s/AKfycbyMVVDXiY91doC5FBeje9UIhvwsZCgm68jDBZ4uKZnE-zRyG6BLUlWMfqt9sO1v0SfcSw/exec"
    );
    const payroll_transaction_response_promise = fetch(
      "https://script.google.com/macros/s/AKfycbzYyGI99twep_njWGI4_Cd7Gd6bBERGhpP3IuwFRtUZ6hXYD-mpP2gRse2KEqjqKyNckw/exec"
    );
    const payroll_summary_response_promise = fetch(
      "https://script.google.com/macros/s/AKfycbxOfQ8EeEUegve06wUBw4afHRkHSnTP4QwnB8N9AKyHfp8x00pDhLkGiCKkECUm2Iuy2g/exec"
    );
    ...
    const employee_contract_data_list =
      await employee_contracts_response.json();
    const payroll_transaction_data_list =
      await payroll_transaction_response.json();
    ...
    user_sidebar.innerHTML = `<u>${
      username_data_list.content[9][findTextInArray(username_data_list, "NAME")]
    }</u>`;
    ...
    const project_based_employee = hr_dashboard.querySelector(
      "#project_based_employee"
    );
    ...
    for (let x = 1; x < employee_data_list.content.length; x++) {
      if (
        ...
        var full_name = findEmployeeName(
          employee_data_list.content[x][
    ...
    total_employee.innerText = `${
      regular_employee_male + project_based_employee_male + subcon_employee_male
    }/${
      regular_employee_female +
      project_based_employee_female +
      subcon_employee_female
    }/${
      regular_employee_name.length +
      project_based_employee_name.length +
      subcon_employee_name.length
    }`;

    // employee_list_section
    const employee_list = document.querySelector(
      "#employee_list_section #employee_list"
    );
    const contract_list = document.querySelector(
      "#contracts_section #contract_list"
    );
    ...
    Object.keys(done_object).forEach((key) => {
      ...
      var form_no =
        myArray[findTextInArray(employee_contract_data_list, "PCF # / LCF #")];
      var start =
        myArray[
          findTextInArray(employee_contract_data_list, "START OF CONTRACT")
        ];
      var expiration =
        myArray[
          findTextInArray(employee_contract_data_list, "END OF CONTRACT")
      ...
      var department =
        myArray[findTextInArray(employee_contract_data_list, "DEPARTMENT")];
      var designation =
        myArray[findTextInArray(employee_contract_data_list, "DESIGNATION")];
      var daily_rate =
        myArray[findTextInArray(employee_contract_data_list, "DAILY RATE")];
    ...
    var options = {
      series: [
        regular_employee_male + regular_employee_female,
        project_based_employee_male + project_based_employee_female,
        subcon_employee_male + subcon_employee_female,
      ],
      chart: {
    ...
    const regular_contract_button = contracts_section.querySelector(
      "#regular_contract_button"
    );
    const project_based_contract_button = contracts_section.querySelector(
      "#project_based_contract_button"
    );
    const logistics_contract_button = contracts_section.querySelector(
      "#logistics_contract_button"
    );
    const regular_contract_form = contracts_section.querySelector(
      "#regular_contract_form"
    );
    const project_based_contract_form = contracts_section.querySelector(
      "#project_based_contract_form"
    );
    const logistics_contract_form = contracts_section.querySelector(
    ...
    logistics_contract_button.addEventListener("click", () => {
      if (logistics_contract_form.style.display == "block") {
        logistics_contract_form.style.display = "none";
      } else {
    ...
    // regular_contract_form
    const search_employee_id_button_reg = regular_contract_form.querySelector(
      "#search_employee_id_button"
    );
    ...
    const civil_status_reg =
      regular_contract_form.querySelector("#civil_status");
    ...
    const spouse_name_container_reg = regular_contract_form.querySelector(
      "#spouse_name_container"
    );
    const affix_reg = regular_contract_form.querySelector("#affix");
    const affix_container_reg =
      regular_contract_form.querySelector("#affix_container");
    const employee_type_reg =
      regular_contract_form.querySelector("#employee_type");
    const details_reg = regular_contract_form.querySelector("#details");
    const start_of_contract_reg =
      regular_contract_form.querySelector("#start_of_contract");
    ...
    const employee_name_reg =
      regular_contract_form.querySelector("#employee_name");
    const employee_name2_reg =
      regular_contract_form.querySelector("#employee_name2");
    ...
    const generate_button_reg =
      regular_contract_form.querySelector("#generate_button");
    const download_button_reg =
      regular_contract_form.querySelector("#download_button");
    const download_and_submit_button_reg = regular_contract_form.querySelector(
      "#download_and_submit_button"
    );
    const contract_container_reg = regular_contract_form.querySelector(
      "#contract_container"
    );
    ...
    search_employee_id_button_reg.addEventListener("click", () => {
      for (let x = 1; x < employee_data_list.content.length; x++) {
        ...
        if (employee_id_reg.value == employee_id_data2) {
          ...
          civil_status_data =
            employee_data_list.content[x][
              findTextInArray(employee_data_list, "CIVIL STATUS")
            ];
          employee_type_data =
            employee_data_list.content[x][
              findTextInArray(employee_data_list, "EMPLOYEE TYPE")
          ...
          daily_rate_data =
            employee_data_list.content[x][
              findTextInArray(employee_data_list, "DAILY RATE")
            ];
          department_data =
            employee_data_list.content[x][
              findTextInArray(employee_data_list, "DEPARTMENT")
          ...
          address_data =
            employee_data_list.content[x][
              findTextInArray(employee_data_list, "PERMANENT ADDRESS")
            ];
          if (gender_data == "MALE") {
    ...
    generate_button_reg.addEventListener("click", () => {
      ...
      if (fixed.checked) {
        var content_data = `As a regular employee, you will continue to receive the daily rate of <b>${amountInWords(
          parseFloat(daily_rate_data)
        )} (${formatNumber(
          parseFloat(daily_rate_data)
        )})</b> for eight hours of work. Your working schedule is from 8:00 AM to 5:00 PM from Monday through Saturday.`;
    ...
    // project_based_contract_form
    const search_employee_id_button_pb =
      project_based_contract_form.querySelector("#search_employee_id_button");
    const employee_id_pb =
      project_based_contract_form.querySelector("#employee_id");
    const gender_pb = project_based_contract_form.querySelector("#gender");
    const civil_status_pb =
      project_based_contract_form.querySelector("#civil_status");
    const first_name_pb =
      project_based_contract_form.querySelector("#first_name");
    const middle_name_pb =
      project_based_contract_form.querySelector("#middle_name");
    const last_name_pb =
      project_based_contract_form.querySelector("#last_name");
    const spouse_name_pb =
      project_based_contract_form.querySelector("#spouse_name");
    const spouse_name_container_pb = project_based_contract_form.querySelector(
      "#spouse_name_container"
    );
    const affix_pb = project_based_contract_form.querySelector("#affix");
    const affix_container_pb =
      project_based_contract_form.querySelector("#affix_container");
    const employee_type_pb =
      project_based_contract_form.querySelector("#employee_type");
    const details_pb = project_based_contract_form.querySelector("#details");
    const start_of_contract_pb =
      project_based_contract_form.querySelector("#start_of_contract");
    const end_of_contract_pb =
      project_based_contract_form.querySelector("#end_of_contract");
    const designation_pb =
      project_based_contract_form.querySelector("#designation");
    const daily_rate_pb =
      project_based_contract_form.querySelector("#daily_rate");
    const department_pb =
      project_based_contract_form.querySelector("#department");
    const first_name2_pb =
      project_based_contract_form.querySelector("#first_name2");
    const middle_name2_pb =
      project_based_contract_form.querySelector("#middle_name2");
    const last_name2_pb =
      project_based_contract_form.querySelector("#last_name2");
    const designation2_pb =
      project_based_contract_form.querySelector("#designation2");
    const date_start_pb =
      project_based_contract_form.querySelector("#date_start");
    const date_expire_pb =
      project_based_contract_form.querySelector("#date_expire");
    const salary_pb = project_based_contract_form.querySelector("#salary");
    const designation3_pb =
      project_based_contract_form.querySelector("#designation3");
    const submit_btn_pb =
      project_based_contract_form.querySelector("#submit_btn");
    const generate_button_pb =
      project_based_contract_form.querySelector("#generate_button");
    const download_button_pb =
      project_based_contract_form.querySelector("#download_button");
    const download_and_submit_button_pb =
      project_based_contract_form.querySelector("#download_and_submit_button");
    const contract_container_pb = project_based_contract_form.querySelector(
      "#contract_container"
    );
    ...
    // logistics_contract_form
    const search_employee_id_button_log = logistics_contract_form.querySelector(
      "#search_employee_id_button"
    );
    const employee_id_log =
      logistics_contract_form.querySelector("#employee_id");
    const gender_log = logistics_contract_form.querySelector("#gender");
    const civil_status_log =
      logistics_contract_form.querySelector("#civil_status");
    const first_name_log = logistics_contract_form.querySelector("#first_name");
    const middle_name_log =
      logistics_contract_form.querySelector("#middle_name");
    const last_name_log = logistics_contract_form.querySelector("#last_name");
    const spouse_name_log =
      logistics_contract_form.querySelector("#spouse_name");
    const spouse_name_container_log = logistics_contract_form.querySelector(
      "#spouse_name_container"
    );
    const affix_log = logistics_contract_form.querySelector("#affix");
    const affix_container_log =
      logistics_contract_form.querySelector("#affix_container");
    const employee_type_log =
      logistics_contract_form.querySelector("#employee_type");
    const details_log = logistics_contract_form.querySelector("#details");
    const start_of_contract_log =
      logistics_contract_form.querySelector("#start_of_contract");
    const end_of_contract_log =
      logistics_contract_form.querySelector("#end_of_contract");
    const designation_log =
      logistics_contract_form.querySelector("#designation");
    ...
    const first_name2_log =
      logistics_contract_form.querySelector("#first_name2");
    const middle_name2_log =
      logistics_contract_form.querySelector("#middle_name2");
    const last_name2_log = logistics_contract_form.querySelector("#last_name2");
    const designation2_log =
      logistics_contract_form.querySelector("#designation2");
    const date_start_log = logistics_contract_form.querySelector("#date_start");
    const date_expire_log =
      logistics_contract_form.querySelector("#date_expire");
    const salary_log = logistics_contract_form.querySelector("#salary");
    const designation3_log =
      logistics_contract_form.querySelector("#designation3");
    const submit_btn_log = logistics_contract_form.querySelector("#submit_btn");
    const generate_button_log =
      logistics_contract_form.querySelector("#generate_button");
    const download_button_log =
      logistics_contract_form.querySelector("#download_button");
    const download_and_submit_button_log =
      logistics_contract_form.querySelector("#download_and_submit_button");
    const contract_container_log = logistics_contract_form.querySelector(
      "#contract_container"
    );
    ...
    if (last_row == 0) {
      ...
      for (
        ...
        if (
          employee_contract_data_list.content[x][
            findTextInArray(employee_contract_data_list, "PCF # / LCF #")
          ]
            .toString()
            .substring(0, 3) == "REG"
        ) {
    ...
    // FORM GENERATOR
    const pcf_form_no =
      project_based_contract_form.querySelector("#pcf_form_no");
    var last_row = employee_contract_data_list.content.length - 1;
    var data_info;
    var data_counter = 0;
    var last_counter_year;
    ...
    if (last_counter_year == current_year) {
      ...
      data_counter = (1).toString().padStart(4, "0");
      pcf_form_no.value = `PCF${current_year}${month}${data_counter}`;
    }
    ...
    if (last_counter_year == current_year) {
      ...
      data_counter = (1).toString().padStart(4, "0");
      lcf_form_no.value = `LCF${current_year}${month}${data_counter}`;
    }
    ...
    const payroll_details_data = form_tab.querySelector(
      "#payroll_details_data"
    );
    ...
    for (let x = 1; x < payroll_summary_data_list.content.length; x++) {
      var payroll_record = `
            <tr>
                ...
                <td>${
                  payroll_summary_data_list.content[x][
                    findTextInArray(payroll_summary_data_list, "YEAR")
                  ]
                }</td>
                <td>${
                  payroll_summary_data_list.content[x][
                    findTextInArray(payroll_summary_data_list, "WEEK NUMBER")
                  ]
                }</td>
                <td>${payroll_summary_data_list.content[x][
                  findTextInArray(payroll_summary_data_list, "EMPLOYEE ID")
                ]
                  .toString()
                  .padStart(5, "0")}</td>
                <td>${
                  payroll_summary_data_list.content[x][
                    findTextInArray(payroll_summary_data_list, "FULL NAME")
                  ]
                }</td>
                <td>${formatNumber(
                  payroll_summary_data_list.content[x][
                    findTextInArray(payroll_summary_data_list, "GROSS SALARY")
                  ]
                )}</td>
                <td>${formatNumber(
                  payroll_summary_data_list.content[x][
                    findTextInArray(
                      payroll_summary_data_list,
                      "TOTAL DEDUCTIONS"
                    )
                  ]
                )}</td>
                <td>${formatNumber(
                  payroll_summary_data_list.content[x][
                    findTextInArray(payroll_summary_data_list, "NET SALARY")
                  ]
                )}</td>
            </tr>
            `;
    ...
    const philhealth_deduction = form_tab.querySelector(
    ...
    const bereavement_assistance = form_tab.querySelector(
      ...
      for (let z = 1; z < employee_data_list.content.length; z++) {
        if (
          ...
          if (
            ...
            day_allowance.value =
              employee_data_list.content[z][
                findTextInArray(employee_data_list, "DAY ALLOWANCE")
              ];
            night_allowance.value =
              employee_data_list.content[z][
                findTextInArray(employee_data_list, "NIGHT ALLOWANCE")
              ];
            payroll_type.value =
              employee_data_list.content[z][
                findTextInArray(employee_data_list, "PAYROLL TYPE")
              ];
            time_in_sched.value = convertTo24HourFormat(
              time_decoder(
                employee_data_list.content[z][
                  findTextInArray(employee_data_list, "TIME IN")
                ]
              )
            );
            time_out_sched.value = convertTo24HourFormat(
              time_decoder(
                employee_data_list.content[z][
                  findTextInArray(employee_data_list, "TIME OUT")
                ]
              )
            );
    ...
    generate_button.addEventListener("click", () => {
      if (payroll_type.value == "WEEKLY") {
        ...
        payroll_length =
          getNumberOfDays(
            getCutoffPeriodAndMonth(parseInt(cut_off_period.value))[1],
            year.value,
            getCutoffPeriodAndMonth(parseInt(cut_off_period.value))[0]
          ) + 1;
        day_name = getDayNamesForCutoffPeriod(
          getCutoffPeriodAndMonth(parseInt(cut_off_period.value))[1],
          year.value,
          getCutoffPeriodAndMonth(parseInt(cut_off_period.value))[0]
        );
        day_dates = getDatesForCutoffPeriod(
          getCutoffPeriodAndMonth(parseInt(cut_off_period.value))[1],
          year.value,
          getCutoffPeriodAndMonth(parseInt(cut_off_period.value))[0]
        );
    ...
    function insertInputs() {
                    ...
                    <div class="day_input${x}">
                        ...
                        <div class="text-center">
                            <div for="time_in${x}" class="text-center fs-3">
                                <input type="date" class="form-control" id="date_input${x}" name="date_input${x}" readonly value="${
          day_dates[x - 1]
        }">                         
                            </div>
                        </div>
      ...
      for (let x = 1; x <= payroll_length; x++) {
        ...
        sick_leave_box_input[x] = document.querySelector(
          `#sick_leave_box_input${x}`
        );
        sick_leave_box[x] = document.querySelector(`#sick_leave_box${x}`);
        vacation_leave_box_input[x] = document.querySelector(
          `#vacation_leave_box_input${x}`
        );
        vacation_leave_box[x] = document.querySelector(
          `#vacation_leave_box${x}`
        );
        leave_box_input[x] = document.querySelector(`#leave_box_input${x}`);
        leave_box[x] = document.querySelector(`#leave_box${x}`);
        rest_day_box_input[x] = document.querySelector(
          `#rest_day_box_input${x}`
        );
        rest_day_box[x] = document.querySelector(`#rest_day_box${x}`);
        day_input[x] = document.querySelectorAll(`.day_input${x}`);
        half_day_box_input[x] = document.querySelector(
          `#half_day_box_input${x}`
        );
        half_day_box[x] = document.querySelector(`#half_day_box${x}`);
        rest_day_duty_box_input[x] = document.querySelector(
          `#rest_day_duty_box_input${x}`
        );
        rest_day_duty_box[x] = document.querySelector(`#rest_day_duty_box${x}`);
        regular_holiday_box_input[x] = document.querySelector(
          `#regular_holiday_box_input${x}`
        );
        regular_holiday_box[x] = document.querySelector(
          `#regular_holiday_box${x}`
        );
        special_holiday_box_input[x] = document.querySelector(
          `#special_holiday_box_input${x}`
        );
        special_holiday_box[x] = document.querySelector(
          `#special_holiday_box${x}`
        );
        ...
        regular_rate_per_hour[x] = document.querySelector(
          `#regular_rate_per_hour${x}`
        );
        night_rate_per_hour[x] = document.querySelector(
          `#night_rate_per_hour${x}`
        );
        ...
        under_time_deduction[x] = document.querySelector(
          `#under_time_deduction${x}`
        );
        ...
        ot_regular_hour_rate[x] = document.querySelector(
          `#ot_regular_hour_rate${x}`
        );
        ot_night_hour_rate[x] = document.querySelector(
          `#ot_night_hour_rate${x}`
        );
        ot_rate_per_hour[x] = document.querySelector(`#ot_rate_per_hour${x}`);
        ot_night_rate_per_hour[x] = document.querySelector(
          `#ot_night_rate_per_hour${x}`
        );
        ...
        function calculateGrossSalary() {
          if (regular_holiday_box[x].checked == true) {
            if (rest_day_duty_box[x].checked == true) {
              regular_hours_pay[x].value =
                parseFloat(regular_hours[x].value) *
                parseFloat(regular_rate_per_hour[x].value);
              night_hours_pay[x].value =
                parseFloat(night_hours[x].value) *
                parseFloat(night_rate_per_hour[x].value);
              subtotal[x].value = (
                parseFloat(regular_hours_pay[x].value) +
                parseFloat(night_hours_pay[x].value)
              ).toFixed(2);
              if (half_day_box[x].checked == true) {
                late_deduction[x].value =
                  parseFloat(late_mins[x].value) *
                  (parseFloat(subtotal[x].value) / 4 / 60) *
                  2;
                under_time_deduction[x].value =
                  parseFloat(under_time_mins[x].value) *
                  (parseFloat(subtotal[x].value) / 4) *
                  2;
              } else {
                late_deduction[x].value =
                  parseFloat(late_mins[x].value) *
                  (parseFloat(subtotal[x].value) / 8 / 60) *
                  2;
                under_time_deduction[x].value =
                  parseFloat(under_time_mins[x].value) *
                  (parseFloat(subtotal[x].value) / 8) *
                  2;
              }
              regular_pay[x].value = (
                parseFloat(subtotal[x].value) -
                (parseFloat(late_deduction[x].value) +
                  parseFloat(under_time_deduction[x].value)) +
                parseFloat(daily_rate.value) * 1.3
              ).toFixed(2);
              ot_pay[x].value =
                parseFloat(ot_hours[x].value) *
                parseFloat(ot_rate_per_hour[x].value);
              ot_night_pay[x].value =
                parseFloat(ot_night_hours[x].value) *
                parseFloat(ot_night_rate_per_hour[x].value);
              ot_pay_subtotal[x].value = (
                parseFloat(ot_pay[x].value) + parseFloat(ot_night_pay[x].value)
              ).toFixed(2);
              salary_day[x].value = (
                parseFloat(regular_pay[x].value) +
                parseFloat(ot_pay_subtotal[x].value) +
                parseFloat(allowance[x].value)
              ).toFixed(2);
            } else {
              regular_hours_pay[x].value =
                parseFloat(regular_hours[x].value) *
        ...
        function calculateUnderTimeHour() {
          const time_in_sched_value = new Date(
            `2000-01-01T${time_in_sched.value}`
          );
          const time_out_sched_value = new Date(
            `2000-01-01T${time_out_sched.value}`
          );
          const time_out_input_value = new Date(
            `2000-01-01T${time_out_input[x].value}`
          );
          ...
          const time_in_sched_total_mins =
            time_in_sched_hours * 60 + time_in_sched_mins;
          var time_out_sched_total_mins =
            time_out_sched_hours * 60 + time_out_sched_mins;
          var time_out_input_total_mins =
            time_out_input_hours * 60 + time_out_input_mins;
          var under_time = 0;
          ...
          var break_hours =
            parseFloat(day_break[x].value) + parseFloat(night_break[x].value);
          var break_mins = break_hours * 60;
          time_out_sched_total_mins -= break_mins;

          if (time_out_sched_total_mins > time_in_sched_total_mins) {
            if (
              time_out_sched_total_mins > time_out_input_total_mins &&
              time_out_input_total_mins > time_in_sched_total_mins
            ) {
              under_time =
                time_out_sched_total_mins - time_out_input_total_mins;
        ...
        function calculateRatePerHour() {
          regular_rate_per_hour[x].value =
            (parseFloat(daily_rate.value) / 8) * 1;
          night_rate_per_hour[x].value =
            (parseFloat(daily_rate.value) / 8) * 1.1;
        }

        function calculateOtRatePerHour() {
          if (with_ot_box[x].checked == true) {
            ot_rate_per_hour[x].value =
              (parseFloat(daily_rate.value) / 8) * 1.25;
            ot_night_rate_per_hour[x].value =
              (parseFloat(daily_rate.value) / 8) * 1.375;
          } else {
          ...
          if (parseInt(time_in_sched.value.slice(0, 2)) >= 18) {
            if (totalHours > 8) {
              ...
              allowance[x].value =
                ((totalHours -
                  (parseFloat(under_time_mins[x].value) +
                    parseFloat(late_mins[x].value) / 60)) /
                  8) *
                parseFloat(night_allowance.value);
            } else {
              allowance[x].value = 0;
            }
          } else {
              ...
              allowance[x].value =
                ((totalHours -
                  (parseFloat(under_time_mins[x].value) +
                    parseFloat(late_mins[x].value) / 60)) /
                  8) *
                parseFloat(day_allowance.value);
        ...
        function calculateNetSalary() {
          ...
          var additional = 0;
          additional =
            parseFloat(adjustment.value) + parseFloat(cash_advance.value);

          var deduction = 0;
          deduction =
            parseFloat(sss_deduction.value) +
            parseFloat(pag_ibig_deduction.value) +
            parseFloat(philhealth_deduction.value) +
            parseFloat(sss_loan.value) +
            parseFloat(pag_ibig_loan.value) +
            parseFloat(ca_deduction.value) +
            parseFloat(uniform.value) +
            parseFloat(housing.value) +
            parseFloat(st_peter.value) +
            parseFloat(cash_not_return.value) +
            parseFloat(hard_hat.value) +
            parseFloat(safety_shoes.value) +
            parseFloat(over_meals.value) +
            parseFloat(bereavement_assistance.value);
        ...
        function calculateHours() {
          ...
          if (half_day_box[x].checked == true) {
            ...
            if (time_start == time_end) {
              ...
              if (
                time_start < night_hours_end - 24 &&
                time_end < night_hours_end
              ) {
                reg_hours =
                  time_end -
                  time_start -
              ...
              if (
                time_start >= night_hours_end - 24 &&
                time_start <= night_hours_start &&
                time_end <= night_hours_end
              ) {
                reg_hours =
                  time_end - time_start - (time_end - night_hours_start);
              ...
              if (
                time_start >= night_hours_end - 24 &&
                time_end > night_hours_end &&
                time_end <= night_hours_start + 24
              ) {
                reg_hours =
                  time_end - time_start - (night_hours_end - night_hours_start);
              ...
              if (
                ...
                reg_hours =
                  time_end - time_start - (night_hours_end - time_start);
              ...
              // time_start 0-5 / time_end 23
                ...
                reg_hours =
                  ...
                  (night_hours_end -
                    ...
                    (time_end - night_hours_start));
                nd_hours =
                  night_hours_end - time_start + (time_end - night_hours_start);
              }
        ...
        function calculateOvertimeHours() {
          ...
          const scheduleTimeOut = parseFloat(
            roundDownTime(time_out_sched.value)
          );
          ...
          if (actualTime > scheduleTimeOut || actualTime <= scheduleTimeIn) {
            ...
            // Handle cases where actualTime crosses midnight
              ...
              if (
                scheduleTimeOut < NIGHT_END_HOUR - 24 &&
                actualTime > NIGHT_START_HOUR &&
                actualTime <= NIGHT_END_HOUR
              ) {
              ...
              // start 6 - 21.5 end 22.5 - 30
                ...
                scheduleTimeOut >= NIGHT_END_HOUR - 24 &&
                scheduleTimeOut <= NIGHT_START_HOUR &&
                actualTime > NIGHT_START_HOUR &&
                actualTime <= NIGHT_END_HOUR
              ) {
                regularOvertimeHours =
              ...
              // start 6 - 21.5 end 30.5 - 46
                ...
                scheduleTimeOut <= NIGHT_START_HOUR &&
                actualTime > NIGHT_END_HOUR &&
                actualTime <= NIGHT_START_HOUR + 24
              ) {
                regularOvertimeHours =
                  actualTime -
                  scheduleTimeOut -
              ...
              // start 22 - 23.5 end 30.5 - 46
                ...
                regularOvertimeHours =
                  actualTime -
                  scheduleTimeOut -
                  (NIGHT_END_HOUR - scheduleTimeOut);
                nightDifferentialOvertimeHours =
                  NIGHT_END_HOUR - scheduleTimeOut;
              }
              ...
              if (
                scheduleTimeOut < NIGHT_END_HOUR &&
                actualTime <= NIGHT_END_HOUR
              ) {
                nightDifferentialOvertimeHours = actualTime - scheduleTimeOut;
              }
          ...
          if (with_ot_box[x].checked == true) {
            ot_hours[x].value =
              regularOvertimeHours - parseFloat(ot_day_break[x].value);
            ot_night_hours[x].value =
              nightDifferentialOvertimeHours -
              parseFloat(ot_night_break[x].value);
          }
        }
        ...
        function calculateRegularHoliday() {
          if (regular_holiday_box[x].checked == true) {
            ...
            if (with_ot_box[x].checked == true) {
              ot_rate_per_hour[x].value =
                (parseFloat(daily_rate.value) / 8) * 2.6;
              ot_night_rate_per_hour[x].value =
                (parseFloat(daily_rate.value) / 8) * 2.86;
            } else {
              ot_rate_per_hour[x].value = 0;
              ot_night_rate_per_hour[x].value = 0;
            }
            if (rest_day_duty_box[x].checked == true) {
              ...
              if (with_ot_box[x].checked == true) {
                ot_rate_per_hour[x].value =
                  (parseFloat(daily_rate.value) / 8) * 3.38;
                ot_night_rate_per_hour[x].value =
                  (parseFloat(daily_rate.value) / 8) * 3.718;
              }
            }
          } else {
            ...
            if (rest_day_duty_box[x].checked == true) {
              regular_rate_per_hour[x].value =
                (parseFloat(daily_rate.value) / 8) * 1.3;
              night_rate_per_hour[x].value =
                (parseFloat(daily_rate.value) / 8) * 1.43;
              ...
              if (with_ot_box[x].checked == true) {
                ot_rate_per_hour[x].value =
                  (parseFloat(daily_rate.value) / 8) * 1.69;
                ot_night_rate_per_hour[x].value =
                  (parseFloat(daily_rate.value) / 8) * 1.859;
              } else {
                ot_rate_per_hour[x].value =
                  (parseFloat(daily_rate.value) / 8) * 1.69;
                ot_night_rate_per_hour[x].value =
        ...
        function calculateSpecialHoliday() {
          if (special_holiday_box[x].checked == true) {
            ...
            if (rest_day_duty_box[x].checked == true) {
              regular_rate_per_hour[x].value =
                (parseFloat(daily_rate.value) / 8) * 1.5;
              night_rate_per_hour[x].value =
                (parseFloat(daily_rate.value) / 8) * 1.65;
              ...
              if (with_ot_box[x].checked == true) {
                ot_rate_per_hour[x].value =
                  (parseFloat(daily_rate.value) / 8) * 1.95;
                ot_night_rate_per_hour[x].value =
                  (parseFloat(daily_rate.value) / 8) * 2.145;
              }
            }
          } else {
            ...
            if (
              half_day_box[x].checked == true ||
              rest_day_duty_box[x].checked == true
            ) {
            } else {
        ...
        absent_box[x].addEventListener("click", () => {
          if (absent_box[x].checked == true) {
            ...
            day_input[x].forEach((otherInput) => {
            ...
            const day_input_number_elements = day[x].querySelectorAll(
              "input[type='number']"
            );
            const day_input_time_elements =
              day[x].querySelectorAll("input[type='time']");
            ...
            if (
              regular_holiday_box[x].checked == true ||
              (special_holiday_box[x].checked == true &&
                payroll_type.value == "SEMI-MONTHLY")
            ) {
        ...
        half_day_box[x].addEventListener("click", () => {
          if (half_day_box[x].checked == true) {
            ...
            if (
              regular_holiday_box[x].checked == true &&
              rest_day_duty_box[x].checked == false
            ) {
              absent_box_input[x].classList.remove("disabled");
            } else if (
              regular_holiday_box[x].checked == true &&
        ...
        time_in_input[x].addEventListener("change", () => {
          ...
          const time_in_input_value = new Date(
            `2000-01-01T${time_in_input[x].value}`
          );
          const time_in_sched_hours = time_in_sched_value.getHours();
          const time_in_sched_mins = time_in_sched_value.getMinutes();
          ...
          const time_in_sched_total_mins =
            time_in_sched_hours * 60 + time_in_sched_mins;
          const time_in_input_total_mins =
            time_in_input_hours * 60 + time_in_input_mins;
          var late = 0;
        ...
        day_break[x].addEventListener("keyup", () => {
          regular_hours[x].value =
            parseFloat(regular_hours[x].value) - parseFloat(day_break[x].value);
          calculateHours();
          calculateAllowance();
          calculateGrossSalary();
          calculateNetSalary();
        ...
        night_break[x].addEventListener("keyup", () => {
          night_hours[x].value =
            parseFloat(night_hours[x].value) - parseFloat(night_break[x].value);
          calculateHours();
          calculateAllowance();
          calculateGrossSalary();
          calculateNetSalary();
        ...
        ot_day_break[x].addEventListener("keyup", () => {
          ot_hours[x].value =
            parseFloat(ot_hours[x].value) - parseFloat(ot_day_break[x].value);
    ...
    const payroll_transaction_button = attendance_form.querySelector(
      "#payroll_transaction_button"
    );
    ...
    const generate_payslip_button = payroll_tab.querySelector(
      "#generate_payslip_button"
    );
    const download_payslip_button = payroll_tab.querySelector(
    ...
    generate_payslip_button.addEventListener("click", () => {
      ...
      if (payroll_type2.value == "WEEKLY") {
        ...
        const search_week_number = attendance_form.querySelector(
          "#search_week_number"
        );
        for (let c = 1; c < payroll_transaction_data_list.content.length; c++) {
          if (
            payroll_transaction_data_list.content[c][
              findTextInArray(payroll_transaction_data_list, "YEAR")
            ] == search_year.value &&
            payroll_transaction_data_list.content[c][
              findTextInArray(
                payroll_transaction_data_list,
                "WEEK NUMBER / MONTH-CUT OFF"
              )
            ] == search_week_number.value
          ) {
            if (
              !payslip_employee_id.includes(
                payroll_transaction_data_list.content[c][
                  findTextInArray(payroll_transaction_data_list, "EMPLOYEE ID")
                ]
              )
            ) {
              payslip_employee_id.push(
                payroll_transaction_data_list.content[c][
                  findTextInArray(payroll_transaction_data_list, "EMPLOYEE ID")
                ]
              );
        ...
        var month =
          new Date(
            getDatesInWeek(search_year.value, search_week_number.value)[0]
          ).getMonth() - 1;
        var year = new Date(
          getDatesInWeek(search_year.value, search_week_number.value)[0]
        ).getFullYear();
        ...
        for (let x = 0; x < payslip_employee_id.length; x += 2) {
          ...
          for (let c = 1; c < payroll_summary_data_list.content.length; c++) {
            if (
              payroll_summary_data_list.content[c][
                findTextInArray(payroll_summary_data_list, "EMPLOYEE ID")
              ] == payslip_employee_id[x] &&
              payroll_summary_data_list.content[c][
                findTextInArray(payroll_summary_data_list, "YEAR")
              ] == search_year.value &&
              payroll_summary_data_list.content[c][
              ...
              adjustment =
                payroll_summary_data_list.content[c][
                  findTextInArray(payroll_summary_data_list, "ADJUSTMENT")
                ];
              cash_advance =
                payroll_summary_data_list.content[c][
                  findTextInArray(payroll_summary_data_list, "CASH ADVANCE")
                ];
              sss =
                payroll_summary_data_list.content[c][
                  findTextInArray(payroll_summary_data_list, "SSS")
                ];
              pag_ibig =
                payroll_summary_data_list.content[c][
                  findTextInArray(payroll_summary_data_list, "PAG-IBIG")
                ];
              philhealth =
                payroll_summary_data_list.content[c][
                  findTextInArray(payroll_summary_data_list, "PHILHEALTH")
                ];
              sss_loan =
                payroll_summary_data_list.content[c][
                  findTextInArray(payroll_summary_data_list, "SSS LOAN")
                ];
              pag_ibig_loan =
                payroll_summary_data_list.content[c][
                  findTextInArray(payroll_summary_data_list, "PAG-IBIG LOAN")
                ];
              ca_deduction =
                payroll_summary_data_list.content[c][
                  findTextInArray(payroll_summary_data_list, "C/A DEDUCTION")
                ];
              uniform =
                payroll_summary_data_list.content[c][
                  findTextInArray(payroll_summary_data_list, "UNIFORM")
                ];
              housing =
                payroll_summary_data_list.content[c][
                  findTextInArray(payroll_summary_data_list, "HOUSING")
                ];
              st_peter =
                payroll_summary_data_list.content[c][
                  findTextInArray(payroll_summary_data_list, "ST. PETER")
                ];
              cash_not_return =
                payroll_summary_data_list.content[c][
                  findTextInArray(payroll_summary_data_list, "CASH NOT RETURN")
                ];
              hard_hat =
                payroll_summary_data_list.content[c][
                  findTextInArray(payroll_summary_data_list, "HARD HAT")
                ];
              safety_shoes =
                payroll_summary_data_list.content[c][
                  findTextInArray(payroll_summary_data_list, "SAFETY SHOES")
                ];
              over_meals =
                payroll_summary_data_list.content[c][
                  findTextInArray(payroll_summary_data_list, "OVER MEALS")
                ];
              bereavement_assistance =
                payroll_summary_data_list.content[c][
                  findTextInArray(
              ...
              sub_deduction =
                sss + sss_loan + pag_ibig + pag_ibig_loan + philhealth;
          ...
          for (
            let c = 1;
            c < payroll_transaction_data_list.content.length;
            c++
          ) {
            if (
              payroll_transaction_data_list.content[c][
              ...
              daily_rate =
                payroll_transaction_data_list.content[c][
                  findTextInArray(payroll_transaction_data_list, "DAILY RATE")
                ];
              allowance +=
                payroll_transaction_data_list.content[c][
                  findTextInArray(payroll_transaction_data_list, "ALLOWANCE")
                ];
              under_time_deduction +=
                payroll_transaction_data_list.content[c][
                  findTextInArray(
                    payroll_transaction_data_list,
              ...
              late_mins +=
                payroll_transaction_data_list.content[c][
                  findTextInArray(payroll_transaction_data_list, "LATE (MINS)")
                ];
              ...
              if (
                payroll_transaction_data_list.content[c][
                  findTextInArray(payroll_transaction_data_list, "DURATION")
                ] != "ABSENT"
              ) {
                var type_of_day =
                  payroll_transaction_data_list.content[c][
                ...
                var day_hours =
                  payroll_transaction_data_list.content[c][
                    findTextInArray(payroll_transaction_data_list, "DAY HOURS")
                  ];
                var ot_day_hours =
                  payroll_transaction_data_list.content[c][
                    findTextInArray(
                ...
                ord_day_hrs_pay =
                  (parseFloat(ord_day_hrs) * parseFloat(daily_rate)) / 8;
                ord_ot_day_hrs_pay =
                  (parseFloat(ord_ot_day_hrs) *
                    (parseFloat(daily_rate) * 1.25)) /
                  8;
                ord_night_hrs_pay =
                  (parseFloat(ord_night_hrs) * (parseFloat(daily_rate) * 1.1)) /
                  8;
                ord_ot_night_hrs_pay =
                  (parseFloat(ord_ot_night_hrs) *
                    (parseFloat(daily_rate) * 1.375)) /
                  8;
                rdd_day_hrs_pay =
                  (parseFloat(rdd_day_hrs) * (parseFloat(daily_rate) * 1.3)) /
                  8;
                rdd_ot_day_hrs_pay =
                  (parseFloat(rdd_ot_day_hrs) *
                    (parseFloat(daily_rate) * 1.69)) /
                  8;
                rdd_night_hrs_pay =
                  (parseFloat(rdd_night_hrs) *
                    (parseFloat(daily_rate) * 1.43)) /
                  8;
                rdd_ot_night_hrs_pay =
                  (parseFloat(rdd_ot_night_hrs) *
                    (parseFloat(daily_rate) * 1.859)) /
                  8;
                sh_day_hrs_pay =
                  (parseFloat(sh_day_hrs) * (parseFloat(daily_rate) * 1.3)) / 8;
                sh_ot_day_hrs_pay =
                  (parseFloat(sh_ot_day_hrs) *
                    (parseFloat(daily_rate) * 1.69)) /
                  8;
                sh_night_hrs_pay =
                  (parseFloat(sh_night_hrs) * (parseFloat(daily_rate) * 1.43)) /
                  8;
                sh_ot_night_hrs_pay =
                  (parseFloat(sh_ot_night_hrs) *
                    (parseFloat(daily_rate) * 1.859)) /
                  8;
                rh_day_hrs_pay =
                  (parseFloat(rh_day_hrs) * (parseFloat(daily_rate) * 2)) / 8;
                rh_ot_day_hrs_pay =
                  (parseFloat(rh_ot_day_hrs) * (parseFloat(daily_rate) * 2.6)) /
                  8;
                rh_night_hrs_pay =
                  (parseFloat(rh_night_hrs) * (parseFloat(daily_rate) * 2.2)) /
                  8;
                rh_ot_night_hrs_pay =
                  (parseFloat(rh_ot_night_hrs) *
                    (parseFloat(daily_rate) * 2.86)) /
                  8;
                sh_rdd_day_hrs_pay =
                  (parseFloat(sh_rdd_day_hrs) *
                    (parseFloat(daily_rate) * 1.5)) /
                  8;
                sh_rdd_ot_day_hrs_pay =
                  (parseFloat(sh_rdd_ot_day_hrs) *
                    (parseFloat(daily_rate) * 1.95)) /
                  8;
                sh_rdd_night_hrs_pay =
                  (parseFloat(sh_rdd_night_hrs) *
                    (parseFloat(daily_rate) * 1.65)) /
                  8;
                sh_rdd_ot_night_hrs_pay =
                  (parseFloat(sh_rdd_ot_night_hrs) *
                    (parseFloat(daily_rate) * 2.145)) /
                  8;
                rh_rdd_day_hrs_pay =
                  (parseFloat(rh_rdd_day_hrs) *
                    (parseFloat(daily_rate) * 2.6)) /
                  8;
                rh_rdd_ot_day_hrs_pay =
                  (parseFloat(rh_rdd_ot_day_hrs) *
                    (parseFloat(daily_rate) * 3.38)) /
                  8;
                rh_rdd_night_hrs_pay =
                  (parseFloat(rh_rdd_night_hrs) *
                    (parseFloat(daily_rate) * 2.86)) /
                  8;
                rh_rdd_ot_night_hrs_pay =
                  (parseFloat(rh_rdd_ot_night_hrs) *
                    (parseFloat(daily_rate) * 3.718)) /
                  8;
              }
            }
          }
                            ...
                            <div class="summary">
                                ...
                                <div class="employee_details">
                                    <div class="grid1">
                                        ...
                                        <div class="d-flex">
                                            <h4 class="bold">Pay Date:</h4><h4 class="ps-1 truncate_text">${
                                              pay_date.value
                                            }</h4>
                                        </div>
                                    </div>
                                ...
                                <div class="summary_details_details">
                                    <div class="border-right border-left">
                                        ...
                                        <h4 style="text-align: right;">${formatNumber(
                                          ord_day_hrs_pay
                                        )}</h4>
                                ...
                                <div class="summary_details_details2">
                                    <div class="border-right border-left">
                                        <h4>SSS ${government_dues_month.substring(
                                          0,
                                          3
                                        )} ${government_dues_year}</h4>
                                        <h4 style="text-align: right; padding-right: 5px;">${formatNumber(
                                          sss
                                        )}</h4>
                                        <h4>HDMF ${government_dues_month.substring(
                                          0,
                                          3
                                        )} ${government_dues_year}</h4>
                                        <h4 style="text-align: right; padding-right: 5px;">${formatNumber(
                                        ...
                                        <h4>PHIC ${government_dues_month.substring(
                                          0,
                                          3
                                        )} ${government_dues_year}</h4>
                                        <h4 style="text-align: right; padding-right: 5px;">${formatNumber(
                                ...
                                <div class="summary_footer border-left border-right">
                                    <div>
                                        ...
                                        <div>
                                            ...
                                            <h3 style="text-align: right; padding-right: 5px;">${formatNumber(
                                              gross_salary
                                            )}</h3>
                                            <h3 style="text-align: right; padding-right: 5px;">${formatNumber(
                                              sub_deduction
                                    ...
                                    <div style="padding: 0 3px; height: 70px;">
                                        ...
                                        <div class="border">
                                            <div class="border-bottom" style="display: grid; grid-template-columns: 50% 50%; align-items: center;">
                                                <h4 class="bold ps-1 truncate_text" style="text-align: start;">Net Pay:</h4><h3 class="pe-1 bold" style="text-align: end;">${formatNumber(
                                                  subtotal
                                                )}</h3>
                                            </div>
                                            <div class="border-bottom" style="display: grid; grid-template-columns: 50% 50%; align-items: center;">
                                                <h4 class="bold ps-1 truncate_text" style="text-align: start;">Other<br>Deductions:</h4><h3 class="pe-1 bold" style="text-align: end;">${formatNumber(
                                                  other_deduction
                                                )}</h3>
                                            </div>
                                            <div class="" style="display: grid; grid-template-columns: 50% 50%; align-items: center;">
                                                <h4 class="bold ps-1 truncate_text" style="text-align: start;">Remaining<br>Salary:</h4><h2 class="pe-1 bold" style="text-align: end;">${formatNumber(
                                                  subtotal - other_deduction
                                                )}</h2>
                                            </div>
                                        </div>
                                    </div>
        ...
        for (let x = 0; x < Math.floor(payslip_employee_id.length / 2); x++) {
          var payslip_container_data = `
                    ...
                    `;
          payslip_container.insertAdjacentHTML(
            "beforeend",
            payslip_container_data
          );
        }
        ...
        const search_year = attendance_form.querySelector("#search_year");
        const search_cut_off_period = attendance_form.querySelector(
          "#search_cut_off_period"
        );
        for (let c = 1; c < payroll_transaction_data_list.content.length; c++) {
          if (
            payroll_transaction_data_list.content[c][
            ...
            ] == parseInt(search_cut_off_period.value)
          ) {
            if (
              !payslip_employee_id.includes(
                payroll_transaction_data_list.content[c][
        ...
        var month = months.indexOf(
          getCutoffPeriodAndMonth(parseInt(search_cut_off_period.value))[1]
        );
        ...
        for (let x = 0; x < payslip_employee_id.length; x += 2) {
          ...
          var employee_designation = "";
          var employee_tin_no = "";
          var pay_period_dates = getDatesForCutoffPeriod(
            getCutoffPeriodAndMonth(parseInt(search_cut_off_period.value))[1],
            search_year.value,
            getCutoffPeriodAndMonth(parseInt(search_cut_off_period.value))[0]
          );
          const parts = date_decoder(pay_period_dates[0]).split(", ");
          var date_start = parts.slice(0, -1).join(", ");
          var pay_period = `${date_start} - ${date_decoder(
        ...
        for (let x = 1; x < payslip_employee_id.length; x += 2) {
          ...
          var date_start = parts.slice(0, -1).join(", ");
          var pay_period = `${date_start} - ${date_decoder(
            pay_period_dates[pay_period_dates.length]
          )}`;
          for (let c = 1; c < employee_data_list.content.length; c++) {
            if (
              employee_id ==
    ...
    const new_employee_button = employee_form.querySelector(
      "#new_employee_button"
    );
    const resignation_form = employee_form.querySelector("#resignation_form");
    const update_record_button = employee_form.querySelector(
    ...
    const new_spouse_name_container = new_employee_form.querySelector(
      "#spouse_name_container"
    );
    const new_affix_container =
      new_employee_form.querySelector("#affix_container");
    const new_employee_type = new_employee_form.querySelector("#employee_type");
    const update_record_form = employee_form.querySelector(
      "#update_record_form"
    );
    const update_employee_id = update_record_form.querySelector("#employee_id");
    const update_search_employee_id_button = update_record_form.querySelector(
      "#search_employee_id_button"
    );
    const update_details = update_record_form.querySelector("#details");
    const update_gender = update_record_form.querySelector("#gender");
    const update_civil_status =
      update_record_form.querySelector("#civil_status");
    ...
    const update_spouse_name_container = update_record_form.querySelector(
      "#spouse_name_container"
    );
    const update_affix = update_record_form.querySelector("#affix");
    const update_affix_container =
      update_record_form.querySelector("#affix_container");
    ...
    const update_mobile_number =
      update_record_form.querySelector("#mobile_number");
    const update_email_address =
      update_record_form.querySelector("#email_address");
    ...
    const update_other_address =
      update_record_form.querySelector("#other_address");
    const update_mothers_maiden_name = update_record_form.querySelector(
      "#mothers_maiden_name"
    );
    const update_educational_attainment = update_record_form.querySelector(
    ...
    const update_philhealth_no =
      update_record_form.querySelector("#philhealth_no");
    const update_pag_ibig_no = update_record_form.querySelector("#pag_ibig_no");
    const update_drivers_license_no = update_record_form.querySelector(
      "#drivers_license_no"
    );
    const update_nbi_no = update_record_form.querySelector("#nbi_no");
    const update_police_clearance_no = update_record_form.querySelector(
      "#police_clearance_no"
    );
    ...
    const update_employee_type =
      update_record_form.querySelector("#employee_type");
    const update_payroll_type =
      update_record_form.querySelector("#payroll_type");
    const update_salary_type = update_record_form.querySelector("#salary_type");
    const update_employee_status =
      update_record_form.querySelector("#employee_status");
    ...
    const update_day_allowance =
      update_record_form.querySelector("#day_allowance");
    const update_night_allowance =
      update_record_form.querySelector("#night_allowance");
    const update_time_in_schedule =
      update_record_form.querySelector("#time_in_schedule");
    const update_time_out_schedule =
      update_record_form.querySelector("#time_out_schedule");
    const update_date_resignation =
      update_record_form.querySelector("#date_resignation");
    const reason_resignation = update_record_form.querySelector(
      "#reason_resignation"
    );
    const timestamp = update_record_form.querySelector("#timestamp");
    const resign_search_employee_id_button = resignation_form.querySelector(
      "#search_employee_id_button"
    );
    ...
    const resign_spouse_name_container = resignation_form.querySelector(
      "#spouse_name_container"
    );
    const resign_affix = resignation_form.querySelector("#affix");
    const resign_affix_container =
      resignation_form.querySelector("#affix_container");
    const resign_date_resignation =
      resignation_form.querySelector("#date_resignation");
    const resign_reason_resignation = resignation_form.querySelector(
      "#reason_resignation"
    );
    ...
    update_search_employee_id_button.addEventListener("click", () => {
      for (let a = 1; a < employee_data_list.content.length; a++) {
        if (
          update_employee_id.value ==
          employee_data_list.content[a][
            findTextInArray(employee_data_list, "EMPLOYEE ID")
          ]
        ) {
          update_gender.value =
          ...
          update_birthday.value = formatDateToInputValue(
            employee_data_list.content[a][
              findTextInArray(employee_data_list, "BIRTH DATE")
            ]
          );
          update_birth_place.value =
            employee_data_list.content[a][
              findTextInArray(employee_data_list, "BIRTH PLACE")
            ];
          update_mobile_number.value =
            employee_data_list.content[a][
              findTextInArray(employee_data_list, "MOBILE NO.")
            ];
          update_email_address.value =
            employee_data_list.content[a][
              findTextInArray(employee_data_list, "EMAIL ADDRESS")
            ];
          update_nationality.value =
            employee_data_list.content[a][
              findTextInArray(employee_data_list, "NATIONALITY")
            ];
          update_address.value =
            employee_data_list.content[a][
              findTextInArray(employee_data_list, "PERMANENT ADDRESS")
            ];
          update_other_address.value =
            employee_data_list.content[a][
              findTextInArray(employee_data_list, "OTHER ADDRESS")
            ];
          update_mothers_maiden_name.value =
            employee_data_list.content[a][
              findTextInArray(employee_data_list, "MOTHER'S MAIDEN NAME")
            ];
          update_educational_attainment.value =
            employee_data_list.content[a][
              findTextInArray(employee_data_list, "EDUCATIONAL ATTAINMENT")
            ];
          update_course.value =
            employee_data_list.content[a][
              findTextInArray(employee_data_list, "COURSE")
            ];
          update_year.value =
            employee_data_list.content[a][
              findTextInArray(employee_data_list, "YEAR GRADUATE")
            ];
          update_tin_no.value =
            employee_data_list.content[a][
              findTextInArray(employee_data_list, "TIN NO.")
          ...
          update_philhealth_no.value =
            employee_data_list.content[a][
              findTextInArray(employee_data_list, "PHILHEALTH NO.")
            ];
          update_pag_ibig_no.value =
            employee_data_list.content[a][
              findTextInArray(employee_data_list, "PAG-IBIG NO.")
            ];
          update_drivers_license_no.value =
            employee_data_list.content[a][
              findTextInArray(employee_data_list, "DRIVER'S LICENSE NO.")
            ];
          update_nbi_no.value =
            employee_data_list.content[a][
              findTextInArray(employee_data_list, "NBI NO.")
            ];
          update_police_clearance_no.value =
            employee_data_list.content[a][
              findTextInArray(employee_data_list, "POLICE CLEARANCE NO.")
            ];
          update_cedula_no.value =
            employee_data_list.content[a][
              findTextInArray(employee_data_list, "CEDULA NO.")
            ];
          update_date_hire.value = formatDateToInputValue(
            employee_data_list.content[a][
              findTextInArray(employee_data_list, "DATE HIRE")
            ]
          );
          update_employee_type.value =
            employee_data_list.content[a][
              findTextInArray(employee_data_list, "EMPLOYEE TYPE")
            ];
          update_payroll_type.value =
            employee_data_list.content[a][
              findTextInArray(employee_data_list, "PAYROLL TYPE")
            ];
          update_salary_type.value =
            employee_data_list.content[a][
              findTextInArray(employee_data_list, "SALARY TYPE")
            ];
          update_employee_status.value =
            employee_data_list.content[a][
              findTextInArray(employee_data_list, "EMPLOYEE STATUS")
            ];
          ...
          update_day_allowance.value =
            employee_data_list.content[a][
              findTextInArray(employee_data_list, "DAY ALLOWANCE")
            ];
          update_night_allowance.value =
            employee_data_list.content[a][
              findTextInArray(employee_data_list, "NIGHT ALLOWANCE")
            ];
          update_time_in_schedule.value = formatTimeToInputValue(
            employee_data_list.content[a][
              findTextInArray(employee_data_list, "TIME IN")
            ]
          );
          update_time_out_schedule.value = formatTimeToInputValue(
            employee_data_list.content[a][
              findTextInArray(employee_data_list, "TIME OUT")
            ]
          );
          update_date_resignation.value = formatDateToInputValue(
            employee_data_list.content[a][
              findTextInArray(employee_data_list, "DATE OF RESIGNATION")
            ]
          );
          reason_resignation.value =
            employee_data_list.content[a][
              findTextInArray(employee_data_list, "REASON OF RESIGNATION")
            ];
          timestamp.value = formatDateToInputValue(
            employee_data_list.content[a][
              findTextInArray(employee_data_list, "CREATED AT")
            ]
          );
        }
      }
    ...
    resign_search_employee_id_button.addEventListener("click", () => {
      for (let a = 1; a < employee_data_list.content.length; a++) {
        if (
          ...
          resign_civil_status.value =
            employee_data_list.content[a][
              findTextInArray(employee_data_list, "CIVIL STATUS")
            ];
          resign_first_name.value =
            employee_data_list.content[a][
              findTextInArray(employee_data_list, "FIRST NAME")
            ];
          resign_middle_name.value =
            employee_data_list.content[a][
              findTextInArray(employee_data_list, "MIDDLE NAME")
            ];
          resign_last_name.value =
            employee_data_list.content[a][
              findTextInArray(employee_data_list, "LAST NAME")
            ];
          resign_spouse_name.value =
            employee_data_list.content[a][
              findTextInArray(employee_data_list, "SPOUSE NAME")
            ];
          resign_affix.value =
          ...
          resign_date_resignation.value = formatDateToInputValue(
            employee_data_list.content[a][
              findTextInArray(employee_data_list, "DATE OF RESIGNATION")
            ]
          );
          resign_reason_resignation.value =
            employee_data_list.content[a][
              findTextInArray(employee_data_list, "REASON OF RESIGNATION")
            ];
          resign_timestamp.value = formatDateToInputValue(
            employee_data_list.content[a][
              findTextInArray(employee_data_list, "CREATED AT")
            ]
          );
    ...
    var employee_number = String(
      employee_data_list.content[employee_data_list.content.length - 1][
        findTextInArray(employee_data_list, "EMPLOYEE ID")
      ]
    ); // Convert to a string
    if (employee_number.slice(0, 2) == lastTwoDigitsOfYear) {
      ...
      if (!isNaN(threeDigitValue) && last_three_digit < threeDigitValue) {
        ...
        last_three_digit += 1;
        new_employee_id.value = `${lastTwoDigitsOfYear}${String(
          last_three_digit
        ).padStart(3, "0")}`;
      }
    } else {
      new_employee_id.value = `${lastTwoDigitsOfYear}${String(1).padStart(
        3,
        "0"
      )}`;
    }
