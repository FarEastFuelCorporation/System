document.addEventListener('DOMContentLoaded', async function() {
    try {
        ...
        const user_name = username_data_list.content[4][findTextInArray(username_data_list, "NAME")];
        profile_picture.src = `../images/profile_picture/${username_data_list.content[4][findTextInArray(username_data_list, "PICTURE")]}`;
                    ...
                    for (let x = 1; x < tpf_data_list.content.length; x++) {
                        if(tpf_data_list.content[x][findTextInArray(tpf_data_list, "TPF #")] == item.tpfNumber){
                            if (!done.includes(item.tpfNumber) && input_box.value === findClientName(tpf_data_list.content[x][findTextInArray(tpf_data_list, "CLIENT ID")])) {
                                ...
                                var ltf = "";
                                for(let k = 1; k < wcf_data_list.content.length; k++){
                                    if(tpf_data_list.content[x][findTextInArray(tpf_data_list, "WCF #")] == wcf_data_list.content[k][findTextInArray(wcf_data_list, "WCF #")]){
                                        if((wcf_data_list.content[k][findTextInArray(wcf_data_list, "LTF/ MTF  #")].substring(0,3) == "MTF")){
                                            mtf = wcf_data_list.content[k][findTextInArray(wcf_data_list, "LTF/ MTF  #")];
                                        }else{
                                            ltf = wcf_data_list.content[k][findTextInArray(wcf_data_list, "LTF/ MTF  #")];
                                ...
                                if (waste_description.value == findWasteName(tpf_data_list.content[x][findTextInArray(tpf_data_list, "CLIENT ID")], tpf_data_list.content[x][findTextInArray(tpf_data_list, "WASTE ID")]) &&
                                    ...
                                    tpf_data_list.content[x][findTextInArray(tpf_data_list, "CLIENT ID")] == "C2024013" ){
                                        ...
                                        if(tpf_data_list.content[x][findTextInArray(tpf_data_list, "WASTE ID")] == "W20230002"){
                                            var new_waste_name = ""
                                            var pull_out_form
                                            for(let a = 1; a < wcf_data_list.content.length; a++){
                                                if(wcf_data_list.content[a][findTextInArray(wcf_data_list, "WCF #")] == tpf_data_list.content[x][findTextInArray(tpf_data_list, "WCF #")]){
                                            ...
                                            var steam_no = [];
                                            console.log(tpf_data_list.content[x][findTextInArray(tpf_data_list, "WCF #")])
                                            for(let y = 1; y < wtf_data_list.content.length; y++){
                                                if(wtf_data_list.content[y][findTextInArray(wtf_data_list, "WCF #")] == tpf_data_list.content[x][findTextInArray(tpf_data_list, "WCF #")]){
                                                    if(!steam_no.includes(wtf_data_list.content[y][findTextInArray(wtf_data_list, "STEAM #")])){
                                                        steam_no.push(wtf_data_list.content[y][findTextInArray(wtf_data_list, "STEAM #")])
                                                    }
                                                }
                                            }
                                            var date_finished = ""
                                            for(let y = 1; y < wdf_data_list.content.length; y++){
                                                if(tpf_data_list.content[x][findTextInArray(tpf_data_list, "SF #")] == wdf_data_list.content[y][findTextInArray(wdf_data_list, "WDF #")]){
                                                    date_finished = wdf_data_list.content[y][findTextInArray(wdf_data_list, "DISPOSAL DATE")]
                                                }
                                            }
                                            ...
                                            <tr style="display: grid; grid-template-columns: 70px 1.3fr 180px 50px 60px 110px 90px;">
                                                ...
                                                <td style="font-weight: bold">${formatNumber(cod_weight)} kgs.</td>
                                                <td>${tpf_data_list.content[x][findTextInArray(tpf_data_list, "DESTRUCTION PROCESS")]}</td>
                                                <td>${date_decoder(date_finished)}</td>
                                            </tr>
                                            `;
                                            table_head_data_value = 
                                            `
        ...
        function findMTFNumber(wcf_number){
            ...
            var ltf = "";
            for(let k = 1; k < wcf_data_list.content.length; k++){
                if(wcf_number == wcf_data_list.content[k][findTextInArray(wcf_data_list, "WCF #")]){
                    if((wcf_data_list.content[k][findTextInArray(wcf_data_list, "LTF/ MTF  #")].substring(0,3) == "MTF")){
                        mtf = wcf_data_list.content[k][findTextInArray(wcf_data_list, "LTF/ MTF  #")];
                    }else{
                        ltf = wcf_data_list.content[k][findTextInArray(wcf_data_list, "LTF/ MTF  #")];
