document.addEventListener("DOMContentLoaded", async function () {
  try {
    ...
    user_sidebar.innerHTML = `<u>${
      username_data_list.content[2][findTextInArray(username_data_list, "NAME")]
    }</u>`;
    ...
    const total_sorting = document.querySelector(
      "#sorting_dashboard_section #total"
    );
    const sorted_sorting = document.querySelector(
      "#sorting_dashboard_section #sorted"
    );
    const unsorted_sorting = document.querySelector(
      "#sorting_dashboard_section #unsorted"
    );
    const unsorted_list_sorting = document.querySelector(
      "#sorting_dashboard_section #unsorted_list"
    );
    const sorted_list_sorting = document.querySelector(
      "#sorting_dashboard_section #sorted_list"
    );
    const sorted_items_sorting = document.querySelector(
      "#sorting_dashboard_section #sorted_items"
    );
    const sorted_items_list_sorting = document.querySelector(
      "#sorting_dashboard_section #sorted_items_list"
    );
    ...
    function generatePending() {
      ...
      for (let i = 1; i < wcf_data_list.content.length; i++) {
        if (
          ...
          if (
            !wcf_transaction_sorting.includes(
      ...
      for (let i = 1; i < sf_data_list.content.length; i++) {
        if (
          !wcf_sf_transaction_sorting.includes(
            sf_data_list.content[i][findTextInArray(sf_data_list, "WCF #")]
          ) &&
          month_filter.value ==
            formatMonth(
          ...
          wcf_sf_transaction_sorting.push(
            sf_data_list.content[i][findTextInArray(sf_data_list, "WCF #")]
          );
        } else if (
          !wcf_sf_transaction_sorting.includes(
        ...
        if (sf_data_list.content[i][4] !== "DISCREPANCY") {
          if (
            !sf_transaction_sorting.includes(
      ...
      pending_sorting = wcf_transaction_sorting.filter(
        (element) => !wcf_sf_transaction_sorting.includes(element)
      );
      done_sorting_sorting = wcf_transaction_sorting.filter((element) =>
        wcf_sf_transaction_sorting.includes(element)
      );
      total_sorting.innerText = wcf_transaction_sorting.length;
      unsorted_sorting.innerText =
        wcf_transaction_sorting.length - wcf_sf_transaction_sorting.length;
      sorted_sorting.innerText = wcf_sf_transaction_sorting.length;
      sorted_items_sorting.innerText = sf_transaction_sorting.length;
      var options = {
        series: [
          wcf_transaction_sorting.length - wcf_sf_transaction_sorting.length,
          wcf_sf_transaction_sorting.length,
        ],
        chart: {
          ...
          if (
            ...
            for (let k = 1; k < sf_data_list.content.length; k++) {
              if (
                ...
                sf_data_list.content[k][findTextInArray(sf_data_list, "WCF #")]
              ) {
                finished_date = date_decoder(
                  sf_data_list.content[k][
    ...
    var data_info =
      sf_data_list.content[last_row][findTextInArray(sf_data_list, "SF #")];
    ...
    for (let i = 1; i <= 21; i++) {
      ...
      if (last_counter_year == current_year) {
        sf_form_nos.push(
          `SF${last_counter_year}${month}${(parseInt(data_counter) + i)
            .toString()
            .padStart(4, "0")}`
        );
      } else {
    ...
    // wcf_data_list
    const search_sf_wcf_form_no = document.getElementById(
      "search_sf_wcf_form_no"
    );
    const search_sf_wcf_form_no_button = document.getElementById(
      "search_sf_wcf_form_no_button"
    );
    const sf_wcf_client = document.getElementById("sf_wcf_client");
    const search_sf_wcf_result = document.getElementById(
      "search_sf_wcf_result"
    );
    ...
    const clear_sf_wcf_form_no_button = document.getElementById(
      "clear_sf_wcf_form_no_button"
    );
    ...
    for (let z = 1; z <= itemcounter.length; z++) {
      ...
      destruction_process[z] = document.getElementById(
        `destruction_process${z}`
      );
      ...
      function select(element, index) {
        ...
        var treatment_input = document.getElementById(
          `destruction_process${index}`
        );
        for (let i = 1; i < wcf_data_list.content.length; i++) {
          if (
            search_sf_wcf_form_no.value ==
    ...
    for (let z = 1; z <= itemcounter2.length; z++) {
      search_wrapper2[z] = document.getElementById(
        `search_type_of_treatment${z}`
      );
    ...
    function add_discrepancy_input() {
      ...
      const sf_form_no = document.getElementById(
        `sf_form_no${parseInt(item_counter.value) + 1}`
      );
    ...
    function add_new_item() {
      ...
      if (item_counter_int >= 2 && item_counter_int <= 20) {
        const additional_item = document.getElementById(
          `additional_item${item_counter_int}`
        );
        const type_of_waste = document.getElementById(
          `type_of_waste${item_counter_int}`
        );
        const weight = document.getElementById(`weight${item_counter_int}`);
        const destruction_process = document.getElementById(
          `destruction_process${item_counter_int}`
        );
    ...
    function findWasteName(client_id, waste_id) {
      ...
      for (let c = 1; c < qlf_data_list.content.length; c++) {
        if (
          client_id ==
          qlf_data_list.content[c][findTextInArray(qlf_data_list, "CLIENT ID")]
        ) {
          var waste_id_data_full =
            qlf_data_list.content[c][
              findTextInArray(qlf_data_list, "WASTE ID/ TYPE OF VEHICLE")
            ];
          var waste_id_data = qlf_data_list.content[c][
            findTextInArray(qlf_data_list, "WASTE ID/ TYPE OF VEHICLE")
          ].substring(0, 8);
          var waste =
            qlf_data_list.content[c][
    ...
    report_generate_button.addEventListener("click", () => {
      ...
      for (let x = 1; x < sf_data_list.content.length; x++) {
        var hauling_date = new Date(
          sf_data_list.content[x][findTextInArray(sf_data_list, "HAULING DATE")]
        );
        var wcf_data =
          sf_data_list.content[x][findTextInArray(sf_data_list, "WCF #")];
        var sf_data =
          sf_data_list.content[x][findTextInArray(sf_data_list, "SF #")];
        var date_data =
        ...
        var client_id_data =
          sf_data_list.content[x][findTextInArray(sf_data_list, "CLIENT ID")];
        var waste_name =
          sf_data_list.content[x][findTextInArray(sf_data_list, "WASTE NAME")];
        var weight =
          sf_data_list.content[x][findTextInArray(sf_data_list, "WEIGHT")];
        var process =
          sf_data_list.content[x][
            findTextInArray(
              sf_data_list,
