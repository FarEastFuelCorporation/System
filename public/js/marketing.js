document.addEventListener("DOMContentLoaded", async function () {
  try {
    ...
    const clf_response_promise = fetch(
      "https://script.google.com/macros/s/AKfycbxTthjFI5UO-6LgpCH7bcy7A-c59g83xfiRiWgeBpetpHrPU9TBY7pP-TabLCQXc0zlSQ/exec"
    );
    const ctf_response_promise = fetch(
      "https://script.google.com/macros/s/AKfycby7B-ynU2Om6j9EZeGq4aROFUSMs7qjG3L7g54MSbwmbtzfAjimVJDavmUeJIy74YzDng/exec"
    );
    ...
    const for_hauling_container_marketing =
      document.getElementById("for_hauling");
    const for_receiving_container_marketing =
      document.getElementById("for_receiving");
    const for_warehousing_container_marketing =
      document.getElementById("for_warehousing");
    const for_sorting_container_marketing =
      document.getElementById("for_sorting");
    const for_treatment_container_marketing =
      document.getElementById("for_treatment");
    const for_certification_container_marketing =
      document.getElementById("for_certification");
    const for_billing_container_marketing =
      document.getElementById("for_billing");
    const for_billing_distribution_container_marketing =
      document.getElementById("for_billing_distribution");
    const for_collection_container_marketing =
      document.getElementById("for_collection");
    const generated_income_container_marketing =
      document.getElementById("generated_income");
    ...
    const marketing_transaction_form = document.querySelector(
      "#marketing_transaction_form"
    );
    const calendar_days =
      marketing_transaction_form.querySelector("#calendar_days");
    const calendar_month =
      marketing_transaction_form.querySelector("#calendar_month");
    ...
    function generatePending() {
      ...
      var [
        ...
        forSortingColor,
        forTreatmentColor,
        forCertificationColor,
        forBillingColor,
        forBillingDistributionColor,
        forCollectionColor,
        finishedColor,
      ] = [
      ...
      booked_transactions_marketing.innerText =
        ...
        for_billing_distribution_marketing +
      ...
      for_billing_distribution_container_marketing.innerText =
        for_billing_distribution_marketing;
      for_collection_container_marketing.innerText = for_collection_marketing;
      generated_income_container_marketing.innerText = formatNumber(
        generated_income_marketing
      );
    ...
    search_wrappers.forEach((search_wrapper) => {
      ...
      function select(element) {
        ...
        const submit_to_marketing_transaction_form =
          marketing_transaction_form.querySelector("#submit_to");
        const type_of_vehicle_container_transaction_form =
          marketing_transaction_form.querySelector(
            "#type_of_vehicle_container"
          );
        const add_item_button_marketing_transaction_form =
          marketing_transaction_form.querySelector("#add_item_button");
        const remove_item_button_marketing_transaction_form =
          marketing_transaction_form.querySelector("#remove_item_button");
    ...
    const quotation_no =
      marketing_transaction_form.querySelector("#quotation_no");
    const type_of_waste =
      marketing_transaction_form.querySelector("#type_of_waste");
    const client_id_input =
      marketing_transaction_form.querySelector("#client_id_input");
    ...
    const type_of_waste_container = marketing_transaction_form.querySelector(
      "#type_of_waste_container"
    );
    const type_of_vehicle_container = marketing_transaction_form.querySelector(
      "#type_of_vehicle_container"
    );
    const marketing_transaction_button =
      marketing_transaction_form.querySelector("#marketing_transaction_button");
    const form_tab_marketing_transaction_form =
      marketing_transaction_form.querySelector("#form_tab");
    const close_button =
      marketing_transaction_form.querySelector("#close_button");
    const add_item_button_marketing_transaction_form =
      marketing_transaction_form.querySelector("#add_item_button");
    const remove_item_button_marketing_transaction_form =
      marketing_transaction_form.querySelector("#remove_item_button");
    ...
    function addTransaction() {
      ...
      const transaction_counter = marketing_transaction_form.querySelector(
        "#transaction_counter"
      );
      ...
      const type_of_vehicle = marketing_transaction_form.querySelector(
        `#type_of_vehicle${transaction_counter.value}`
      );
      const submit_to = marketing_transaction_form.querySelector(`#submit_to`);
      var done_vehicle = [];
      for (let y = 1; y < client_data_list.content.length; y++) {
    ...
    remove_item_button_marketing_transaction_form.addEventListener(
      "click",
      () => {
        const marketing_transaction_form = document.querySelector(
          "#marketing_transaction_form"
        ...
        const type_of_vehicle_container =
          marketing_transaction_form.querySelector(
            `#type_of_vehicle_container${transaction_counter.value}`
          );
    ...
    waste_category.addEventListener("change", () => {
      if (waste_category.value == "HAZARDOUS WASTE") {
        ...
        var data = `<div id="pull_out_form_container" style="display: none">
        ...
        var data2 = `<div id="pull_out_form_container">
                    <label for="pull_out_form">
                        <i class="fa-solid fa-right-from-bracket"></i>
                        Pull Out Form #
                    </label>
    ...
    function generateCalendarDays(year, month) {
      const hauling_date =
        marketing_transaction_form.querySelector("#hauling_date");
        ...
        for (var j = 0; j < 7; j++) {
          ...
          if (index >= 0 && index < daysInMonth) {
            ...
            var date_format =
              ...
              "-" +
              dayValues[index].toString().padStart(2, "0");
            ...
            bookSlotDiv.innerHTML = `Booked Transactions:<b>${
              book_logistics + book_receiving
            }</b><br>Logistics:<b>${book_logistics}</b> - Receiving:<b>${book_receiving}</b>`;
            ...
            vehicle_type.forEach((data, index) => {
              ...
              var actual_count = 0;
              if (
                total_vehicle[vehicle_type[index]] -
                  booked_vehicle_day[vehicle_type[index]] <
                0
              ) {
              ...
              vehicle_div.textContent = `${data} : ${actual_count}/${
                total_vehicle[vehicle_type[index]]
              }`;
            ...
            bookButton.addEventListener("click", function (event) {
              ...
              var formattedDate =
                ...
                (month + 1).toString().padStart(2, "0") +
                "-" +
                clickedDate.toString().padStart(2, "0");
    ...
    const new_client_form_tab_client_list_section =
      client_list_section.querySelector("#new_client_form_tab");
    const new_employee_button_client_list_section =
      client_list_section.querySelector("#new_employee_button");
    const update_record_button_client_list_section =
      client_list_section.querySelector("#update_record_button");
    const update_client_form_tab_client_list_section =
      client_list_section.querySelector("#update_client_form_tab");
    const search_client_id_button_update_client_form =
      update_client_form_tab_client_list_section.querySelector(
        "#search_client_id_button"
      );
    const search_client_id_update_client_form =
      update_client_form_tab_client_list_section.querySelector("#client_id");
    const client_name_update_client_form =
      update_client_form_tab_client_list_section.querySelector("#client_name");
    const address_update_client_form =
      update_client_form_tab_client_list_section.querySelector("#address");
    const nature_of_business_update_client_form =
      update_client_form_tab_client_list_section.querySelector(
        "#nature_of_business"
      );
    const contact_number_update_client_form =
      update_client_form_tab_client_list_section.querySelector(
        "#contact_number"
      );
    const tin_no_update_client_form =
      update_client_form_tab_client_list_section.querySelector("#tin_no");
    const biller_name_update_client_form =
      update_client_form_tab_client_list_section.querySelector("#biller_name");
    const biller_address_update_client_form =
      update_client_form_tab_client_list_section.querySelector(
        "#biller_address"
      );
    const biller_contact_person_update_client_form =
      update_client_form_tab_client_list_section.querySelector(
        "#biller_contact_person"
      );
    const biller_contact_number_update_client_form =
      update_client_form_tab_client_list_section.querySelector(
        "#biller_contact_number"
      );
    const biller_nature_of_business_update_client_form =
      update_client_form_tab_client_list_section.querySelector(
        "#biller_nature_of_business"
      );
    const biller_tin_no_update_client_form =
      update_client_form_tab_client_list_section.querySelector(
        "#biller_tin_no"
      );
    const timestamp_update_client_form =
      update_client_form_tab_client_list_section.querySelector("#timestamp");
    const update_client_form_tab_container_client_form =
      update_client_form_tab_client_list_section.querySelector(
        "#update_client_form_tab_container"
      );
    ...
    const new_quotation_button_quotation_form = quotation_form.querySelector(
      "#new_quotation_button"
    );
    const update_quotation_button_quotation_form = quotation_form.querySelector(
      "#update_quotation_button"
    );
    const new_quotation_form_tab = quotation_form.querySelector(
      "#new_quotation_form_tab"
    );
    const update_quotation_form_tab = quotation_form.querySelector(
      "#update_quotation_form_tab"
    );
    const add_item_button_quotation_form =
      new_quotation_form_tab.querySelector("#add_item_button");
    const remove_item_button_quotation_form =
      new_quotation_form_tab.querySelector("#remove_item_button");
    const list_counter_quotation_form =
      new_quotation_form_tab.querySelector("#list_counter");
    const list_container_quotation_form =
      new_quotation_form_tab.querySelector("#list_container");
    const transportation_fee_container_quotation_form =
      new_quotation_form_tab.querySelector("#transportation_fee_container");
    const tf_button_quotation_form =
      new_quotation_form_tab.querySelector("#tf_button");
    const tf_value_quotation_form =
      new_quotation_form_tab.querySelector("#tf_value");
    const tf_counter_quotation_form =
      new_quotation_form_tab.querySelector("#tf_counter");
    const add_tf_button_quotation_form =
      new_quotation_form_tab.querySelector("#add_tf_button");
    const remove_tf_button_quotation_form =
      new_quotation_form_tab.querySelector("#remove_tf_button");
    const with_moa_quotation_form =
      new_quotation_form_tab.querySelector("#with_moa");
    const without_moa_quotation_form =
      new_quotation_form_tab.querySelector("#without_moa");
    const validity_quotation_form =
      new_quotation_form_tab.querySelector("#validity");
    const validity_container_quotation_form =
      new_quotation_form_tab.querySelector("#validity_container");
    const generate_button_quotation_form =
      new_quotation_form_tab.querySelector("#generate_button");
    const quotation_no_quotation_form =
      new_quotation_form_tab.querySelector("#quotation_no");
    const qlf_form_no_container_quotation_form =
      new_quotation_form_tab.querySelector("#qlf_form_no_container");
    const date_made_container_quotation_form =
      new_quotation_form_tab.querySelector("#date_made_container");
    const client_id_input_quotation_form =
      new_quotation_form_tab.querySelector("#client_id_input");
    const client_name_container_quotation_form =
      new_quotation_form_tab.querySelector("#client_name_container");
    const address_container_quotation_form =
      new_quotation_form_tab.querySelector("#address_container");
    const nature_of_business_container_quotation_form =
      new_quotation_form_tab.querySelector("#nature_of_business_container");
    const contact_number_quotation_form =
      new_quotation_form_tab.querySelector("#contact_number");
    const contact_number_container_quotation_form =
      new_quotation_form_tab.querySelector("#contact_number_container");
    const client_name_container_quotation_form2 =
      new_quotation_form_tab.querySelector("#client_name_container2");
    const address_container_quotation_form2 =
      new_quotation_form_tab.querySelector("#address_container2");
    const contact_number2_container_quotation_form2 =
      new_quotation_form_tab.querySelector("#contact_number2_container");
    const nature_of_business_container_quotation_form2 =
      new_quotation_form_tab.querySelector("#nature_of_business_container2");
    const contact_number_quotation_form2 =
      new_quotation_form_tab.querySelector("#contact_number2");
    const table_data_quotation_form =
      new_quotation_form_tab.querySelector("#table_data");
    const validity_input_quotation_form =
      new_quotation_form_tab.querySelector("#validity_input");
    const terms_quotation_form = new_quotation_form_tab.querySelector("#terms");
    const terms2_quotation_form =
      new_quotation_form_tab.querySelector("#terms2");
    const charge_input_quotation_form =
      new_quotation_form_tab.querySelector("#charge_input");
    const buying_input_quotation_form =
      new_quotation_form_tab.querySelector("#buying_input");
    const marketing_user_quotation_form =
      new_quotation_form_tab.querySelector("#marketing_user");
    const marketing_signature_quotation_form =
      new_quotation_form_tab.querySelector("#marketing_signature");
    const accounting_head_signature_quotation_form =
    ...
    const convertToPDF_button = new_quotation_form_tab.querySelector(
      "#convertToPDF_button"
    );
    const submit_button =
      new_quotation_form_tab.querySelector("#submit_button");
    const revision_no_quotation_form =
      new_quotation_form_tab.querySelector("#revision_no");
    const revision_number_container_quotation_form =
      new_quotation_form_tab.querySelector("#revision_number_container");
    const valid_until_container_quotation_form =
      new_quotation_form_tab.querySelector("#valid_until_container");
    const scope_of_work_input_quotation_form =
      new_quotation_form_tab.querySelector("#scope_of_work_input");
    const scope_of_work_quotation_form =
      new_quotation_form_tab.querySelector("#scope_of_work");
    const remarks_input_quotation_form =
      new_quotation_form_tab.querySelector("#remarks_input");
    ...
    generate_button_quotation_form.addEventListener("click", () => {
      scope_of_work_input_quotation_form.innerText =
        scope_of_work_quotation_form.value;
      remarks_input_quotation_form.innerText = remarks_quotation_form.value;
      qlf_form_no_container_quotation_form.innerText =
        quotation_no_quotation_form.value;
      var currentDate = new Date();
      date_made_container_quotation_form.innerText = date_decoder(currentDate);
      currentDate.setDate(currentDate.getDate() + 30);
      valid_until_container_quotation_form.innerText =
        date_decoder(currentDate);
      revision_number_container_quotation_form.innerText =
        revision_no_quotation_form.value.toString().padStart(2, "0");
      ...
      for (let x = 0; x < list_counter_quotation_form.value; x++) {
        const waste_name = new_quotation_form_tab.querySelector(
          `#waste_name${x + 1}`
        );
        const unit = new_quotation_form_tab.querySelector(`#unit${x + 1}`);
        const unit_price = new_quotation_form_tab.querySelector(
          `#unit_price${x + 1}`
        );
        const mode = new_quotation_form_tab.querySelector(`#mode${x + 1}`);
        const vat_calculation = new_quotation_form_tab.querySelector(
          `#vat_calculation${x + 1}`
        );
        var quantity = 0;
        const table_row = table_data_quotation_form.querySelectorAll("tr")[row];
        table_row.querySelectorAll("td")[0].innerText = `${counter}`;
        table_row.querySelectorAll("td")[1].innerText =
          waste_name.value.toUpperCase();
        ...
        table_row.querySelectorAll("td")[4].innerText = formatNumber(
          parseFloat(unit_price.value)
        );
        table_row.querySelectorAll("td")[5].innerText = formatNumber(
      ...
      for (let x = 0; x < tf_counter_quotation_form.value; x++) {
        const type_of_vehicle = new_quotation_form_tab.querySelector(
          `#type_of_vehicle${x + 1}`
        );
        const transportation_fee = new_quotation_form_tab.querySelector(
          `#transportation_fee${x + 1}`
        );
        const tf_unit = new_quotation_form_tab.querySelector(
          `#tf_unit${x + 1}`
        );
        const tf_unit_price = new_quotation_form_tab.querySelector(
          `#tf_unit_price${x + 1}`
        );
        const tf_mode = new_quotation_form_tab.querySelector(
          `#tf_mode${x + 1}`
        );
        const tf_vat_calculation = new_quotation_form_tab.querySelector(
          `#tf_vat_calculation${x + 1}`
        );
        var quantity = 0;
        const table_row = table_data_quotation_form.querySelectorAll("tr")[row];
        table_row.querySelectorAll("td")[0].innerText = `${counter}`;
        table_row.querySelectorAll(
          "td"
        )[1].innerText = `TRANSPORTATION FEE - ${type_of_vehicle.value}`;
        ...
        table_row.querySelectorAll("td")[5].style.paddingRight = `5px`;
        table_row.querySelectorAll("td")[4].innerText = formatNumber(
          parseFloat(tf_unit_price.value)
        );
        table_row.querySelectorAll("td")[5].innerText = formatNumber(
          tf_unit_price.value * quantity
        );
        table_row.querySelectorAll("td")[6].innerText = tf_mode.value;
        table_row.querySelectorAll("td")[7].innerText =
          tf_vat_calculation.value;
        row = row + 1;
        const table_row2 =
          table_data_quotation_form.querySelectorAll("tr")[row];
        table_row2.querySelectorAll(
          "td"
        )[1].innerText = `(AREA OF HAUL - ${transportation_fee.value})`;
      ...
      charge_input_quotation_form.innerText =
        terms_quotation_form.value.toUpperCase();
      buying_input_quotation_form.innerText =
        terms2_quotation_form.value.toUpperCase();
      convertToPDFandDownload_button.style.display = "block";
      convertToPDF_button.style.display = "block";
      submit_button.style.display = "block";
    });

    const search_quotation_no_button_quotation_form =
      update_quotation_form_tab.querySelector("#search_quotation_no_button");
    const search_quotation_no_button_container_quotation_form =
      update_quotation_form_tab.querySelector(
        "#search_quotation_no_button_container"
      );
    const display_input_quotation_form =
      update_quotation_form_tab.querySelectorAll(".display_input");
    const list_container_quotation_form2 =
      update_quotation_form_tab.querySelector("#list_container");
    const transportation_fee_container =
      update_quotation_form_tab.querySelector("#transportation_fee_container");
    const button_container_quotation_form =
      update_quotation_form_tab.querySelector("#button_container");
    const add_item_button_quotation_form2 =
      update_quotation_form_tab.querySelector("#add_item_button");
    const remove_item_button_quotation_form2 =
      update_quotation_form_tab.querySelector("#remove_item_button");
    const list_counter_quotation_form2 =
      update_quotation_form_tab.querySelector("#list_counter");
    const transportation_fee_container_quotation_form2 =
      update_quotation_form_tab.querySelector("#transportation_fee_container");
    const tf_button_quotation_form2 =
      update_quotation_form_tab.querySelector("#tf_button");
    const tf_value_quotation_form2 =
      update_quotation_form_tab.querySelector("#tf_value");
    const tf_counter_quotation_form2 =
      update_quotation_form_tab.querySelector("#tf_counter");
    const add_tf_button_quotation_form2 =
      update_quotation_form_tab.querySelector("#add_tf_button");
    const remove_tf_button_quotation_form2 =
      update_quotation_form_tab.querySelector("#remove_tf_button");
    const quotation_no_quotation_form2 =
      update_quotation_form_tab.querySelector("#quotation_no");
    const timestamp_quotation_form2 =
      update_quotation_form_tab.querySelector("#timestamp");
    const terms_quotation_form2 =
      update_quotation_form_tab.querySelector("#terms");
    const client_quotation_form2 =
      update_quotation_form_tab.querySelector("#client");
    const waste_code1_quotation_form2 =
      update_quotation_form_tab.querySelector("#waste_code1");
    const waste_name1_quotation_form2 =
      update_quotation_form_tab.querySelector("#waste_name1");
    const mode1_quotation_form2 =
      update_quotation_form_tab.querySelector("#mode1");
    const unit1_quotation_form2 =
      update_quotation_form_tab.querySelector("#unit1");
    const unit_price1_quotation_form2 =
      update_quotation_form_tab.querySelector("#unit_price1");
    const vat_calculation1_quotation_form2 =
      update_quotation_form_tab.querySelector("#vat_calculation1");
    const with_moa_quotation_form2 =
      update_quotation_form_tab.querySelector("#with_moa");
    const without_moa_quotation_form2 =
      update_quotation_form_tab.querySelector("#without_moa");
    const validity_quotation_form2 =
      update_quotation_form_tab.querySelector("#validity");
    const validity_container_quotation_form2 =
      update_quotation_form_tab.querySelector("#validity_container");
    ...
    function typeOfVehicleOption() {
      ...
      const type_of_vehicle_containers =
        new_quotation_form_tab.querySelectorAll(
          "#type_of_vehicle_container select"
        );
      for (let x = 1; x < vehicle_data_list.content.length; x++) {
        if (
      ...
      ].insertAdjacentHTML("beforeend", type_of_vehicle_selection);
    }
    typeOfVehicleOption();
    ...
    function typeOfVehicleOption2() {
      ...
      var type_of_vehicle_selection = "";
      const type_of_vehicle_containers =
        update_quotation_form_tab.querySelectorAll(
          "#type_of_vehicle_container select"
        );
      for (let x = 1; x < vehicle_data_list.content.length; x++) {
        if (
    ...
    for (let x = 1; x < qlf_data_list.content.length; x++) {
      if (
        ...
        var qlf_data_value2 = `
                ...
                `;
        qlf_list2_quotation_form.insertAdjacentHTML(
          "beforeend",
          qlf_data_value2
        );
    ...
    add_item_button_quotation_form.addEventListener("click", () => {
      list_counter_quotation_form.value =
        parseInt(list_counter_quotation_form.value) + 1;
      var data = `
    ...
    tf_button_quotation_form.addEventListener("click", () => {
      if (tf_value_quotation_form.value == "false") {
        ...
        var data = `                        
                ...
                `;
        transportation_fee_container_quotation_form.insertAdjacentHTML(
          "beforeend",
          data
        );
        ...
        const transportation_fee_container =
          new_quotation_form_tab.querySelector("#transportation_fee_container");
        const tf_items =
          transportation_fee_container.querySelectorAll(".tf_item");
    ...
    add_tf_button_quotation_form.addEventListener("click", () => {
      tf_counter_quotation_form.value =
        parseInt(tf_counter_quotation_form.value) + 1;
      var data = `                        
    ...
    remove_tf_button_quotation_form.addEventListener("click", () => {
      const transportation_fee_container = new_quotation_form_tab.querySelector(
        "#transportation_fee_container"
      );
    ...
    search_quotation_no_button_quotation_form.addEventListener("click", () => {
      search_quotation_no_button_container_quotation_form.style.display =
        "none";
      add_item_button_quotation_form2.style.display = "block";
      display_input_quotation_form.forEach((input) => {
      ...
      for (let x = 1; x < qlf_data_list.content.length; x++) {
          ...
          if (
            ...
            if (waste_counter == 1) {
              ...
              var waste_code1_quotation_form =
                update_quotation_form_tab.querySelector(
                  `#waste_code${waste_counter}`
                );
              var waste_name1_quotation_form =
                update_quotation_form_tab.querySelector(
                  `#waste_name${waste_counter}`
                );
              var mode1_quotation_form =
                update_quotation_form_tab.querySelector(
                  `#mode${waste_counter}`
                );
              var unit1_quotation_form =
                update_quotation_form_tab.querySelector(
                  `#unit${waste_counter}`
                );
              var unit_price1_quotation_form =
                update_quotation_form_tab.querySelector(
                  `#unit_price${waste_counter}`
                );
              var vat_calculation1_quotation_form =
                update_quotation_form_tab.querySelector(
                  `#vat_calculation${waste_counter}`
                );
        ...
        for (let y = 1; y < vehicle_list.length; y++) {
          if (
            ...
            var waste_code1_quotation_form =
              update_quotation_form_tab.querySelector(
                `#type_of_vehicle${counter}`
              );
            var waste_name1_quotation_form =
              update_quotation_form_tab.querySelector(
                `#transportation_fee${counter}`
              );
            var mode1_quotation_form = update_quotation_form_tab.querySelector(
              `#tf_mode${counter}`
            );
            var unit1_quotation_form = update_quotation_form_tab.querySelector(
              `#tf_unit${counter}`
            );
            var unit_price1_quotation_form =
              update_quotation_form_tab.querySelector(
                `#tf_unit_price${counter}`
              );
            var vat_calculation1_quotation_form =
              update_quotation_form_tab.querySelector(
                `#tf_vat_calculation${counter}`
              );
    ...
    function addQuotationList() {
      list_counter_quotation_form2.value =
        parseInt(list_counter_quotation_form2.value) + 1;
      var data = `
    ...
    function addTransportationFeeList() {
      tf_counter_quotation_form2.value =
        parseInt(tf_counter_quotation_form2.value) + 1;
      var data = `                        
            <h4 style="position: absolute; top: -30px;">TRANSPORTATION FEE</h4>
            ...
            `;
      transportation_fee_container_quotation_form2.insertAdjacentHTML(
        "beforeend",
        data
      );
    ...
    remove_item_button_quotation_form2.addEventListener("click", () => {
      const list =
        list_container_quotation_form2.querySelectorAll(".list_item");
      const last_list = list[list.length - 1];
    ...
    tf_button_quotation_form2.addEventListener("click", () => {
      if (tf_value_quotation_form2.value == "false") {
        ...
        tf_value_quotation_form2.value = "true";
        tf_button_quotation_form2.innerHTML =
          "WITHOUT<br>TRANSPORTATION<br>FEE";
    ...
    //   "#new_commission_button"
    // );
    // const update_commission_button_commission = commission_form.querySelector(
    //   "#update_commission_button"
    // );
    // const commission_transaction_button_commission =
    //   commission_form.querySelector("#commission_transaction_button");
    // const new_commission_form_tab_commission = commission_form.querySelector(
    //   "#new_commission_form_tab"
    // );
    // const update_commission_form_tab_commission = commission_form.querySelector(
    //   "#update_commission_form_tab"
    // );
    // const commission_transaction_form_tab_commission =
    //   commission_form.querySelector("#commission_transaction_form_tab");
    // const add_item_button_commission =
    //   commission_form.querySelector("#add_item_button");
    // const remove_item_button_commission = commission_form.querySelector(
    //   "#remove_item_button"
    // );
    ...
    // var data_info =
    //   clf_data_list.content[last_row][findTextInArray(clf_data_list, "CLF #")];
    // var data_counter;
    // if (last_row == 0) {
    //   data_counter = 0;
    // } else {
    ...
    // const quotation_selection = commission_form.querySelector(
    //   "#quotation_selection"
    // );
    ...
    // remove_item_button_commission.addEventListener(
    //   "click",
    //   removeCommissionList
    // );

    // function addCommissionList() {
    //   const commission_data_counter_commission = commission_form.querySelector(
    //     "#commission_data_counter"
    //   );
    //   const commission_list_commission =
    //     commission_form.querySelector("#commission_list");
    //   const waste_name_selection1 = commission_form.querySelector(
    //     "#waste_name_selection1"
    //   );
    //   commission_data_counter_commission.value =
    //     parseInt(commission_data_counter_commission.value) + 1;
    ...
    //         </div>
    //         `;
    //   commission_list_commission.insertAdjacentHTML(
    //     "beforeend",
    //     commission_data
    //   );
    //   remove_item_button_commission.style.display = "block";

    //   const waste_name_selection = commission_form.querySelector(
    //     `#waste_name_selection${commission_data_counter_commission.value}`
    //   );
    //   const waste_id_commission = commission_form.querySelector(
    //     `#waste_id${commission_data_counter_commission.value}`
    //   );
    ...
    //     "#commission_data_counter"
    //   );
    //   const commission = commission_form.querySelector(
    //     `#commission${commission_data_counter_commission.value}`
    //   );
    //   commission.remove();
    //   commission_data_counter_commission.value =
    //     parseInt(commission_data_counter_commission.value) - 1;
    ...
    //   quotation_selection.insertAdjacentHTML(
    //     "beforeend",
    ...
    //     waste_name_selection1.insertAdjacentHTML(
    //       "beforeend",
    //       `<option value="${data}">${data}</option>`
    //     );
    //   });
    ...
    //             <td>${
    //               clf_data_list.content[x][
    //                 findTextInArray(clf_data_list, "CLF #")
    //               ]
    //             }</td>
    //             <td>${
    //               clf_data_list.content[x][
    //                 findTextInArray(clf_data_list, "QUOTATION CODE")
    //               ]
    //             }</td>
    //             <td>${findEmployeeName(
    //               clf_data_list.content[x][
    //                 findTextInArray(clf_data_list, "AGENT ID")
    //               ]
    //             )}</td>
    //             <td>${findClientName(
    //               clf_data_list.content[x][
    //                 findTextInArray(clf_data_list, "CLIENT ID")
    //               ]
    //             )}</td>
    //             <td>${
    //               clf_data_list.content[x][
    //                 findTextInArray(clf_data_list, "WASTE NAME")
    //               ]
    //             }</td>
    //             <td>${
    //               clf_data_list.content[x][
    //                 findTextInArray(clf_data_list, "COMMISSION PRICE")
    //               ]
    //             }</td>
    //             <td>${
    //               clf_data_list.content[x][
    //                 findTextInArray(clf_data_list, "SUBMITTED BY")
    //               ]
    //             }</td>
    //         </tr>
    //         `;
    ...
    // const marketing_user_commission_form =
    //   commission_form.querySelector("#marketing_user");
    // const marketing_signature_commission_form = commission_form.querySelector(
    //   "#marketing_signature"
    // );
    // const accounting_head_signature_commission_form =
    //   commission_form.querySelector("#accounting_head_signature");
    // const vice_president_signature_commission_form =
    //   commission_form.querySelector("#vice_president_signature");
    // const clf_pending_list_commission_form =
    //   commission_form.querySelector("#clf_pending_list");
    // const generate_button_commission_form =
    //   commission_form.querySelector("#generate_button");
    // const search_bpf_form_no_commission_form = commission_form.querySelector(
    //   "#search_bpf_form_no"
    // );
    // const commission_transaction_form_tab = commission_form.querySelector(
    //   "#commission_transaction_form_tab"
    // );
    // const table_data_commission_form =
    //   commission_transaction_form_tab.querySelector("#table_data");
    // const ctf_form_no_container_commission_form =
    //   commission_transaction_form_tab.querySelector("#ctf_form_no_container");
    // const ctf_form_no_commission_form =
    //   commission_transaction_form_tab.querySelector("#ctf_form_no");
    // const client_name_container_commission_form =
    //   commission_transaction_form_tab.querySelector("#client_name_container");

    // marketing_user_commission_form.innerText = user.value
    //   .toLowerCase()
    //   .replace(/\b\w/g, function (char) {
    //     return char.toUpperCase();
    //   });
    ...
    // for (let x = 1; x < clf_data_list.content.length; x++) {
    //   var client_id_clf =
    //     clf_data_list.content[x][findTextInArray(clf_data_list, "CLIENT ID")];
    //   var waste_id_clf =
    //     clf_data_list.content[x][findTextInArray(clf_data_list, "WASTE ID")];
    //   var quotation_no_clf =
    //     clf_data_list.content[x][
    //       findTextInArray(clf_data_list, "QUOTATION CODE")
    //     ];
    //   var agent_name_clf =
    //     clf_data_list.content[x][findTextInArray(clf_data_list, "AGENT ID")];
    //   var commission_price_clf =
    //     clf_data_list.content[x][
    //       findTextInArray(clf_data_list, "COMMISSION PRICE")
    //     ];
    //   for (let y = 1; y < bpf_data_list.content.length; y++) {
    //     var bpf_no_bpf =
    //       bpf_data_list.content[y][findTextInArray(bpf_data_list, "BPF #")];
    //     var cod_no_bpf =
    //       bpf_data_list.content[y][findTextInArray(bpf_data_list, "COD #")];
    //     var submitted_by_bpf =
    //       bpf_data_list.content[y][
    //         findTextInArray(bpf_data_list, "SUBMITTED BY")
    //       ];
    //     var client_id_cod, waste_id_cod;
    //     for (let z = 1; z < cod_data_list.content.length; z++) {
    //       var cod_no_cod =
    //         cod_data_list.content[z][findTextInArray(cod_data_list, "COD #")];
    //       var wcf_no_cod =
    //         cod_data_list.content[z][findTextInArray(cod_data_list, "WCF #")];
    //       var weight_cod =
    //         cod_data_list.content[z][findTextInArray(cod_data_list, "WEIGHT")];
    //       var mtf = "";
    //       var ltf = "";
    ...
    //           ];
    //         if (
    //           client_id_clf == client_id_cod &&
    //           waste_id_clf == waste_id_cod
    //         ) {
    //           var data_value = `
    //                         <tr>
    ...
    //                             <td>${formatNumber(
    //                               commission_price_clf * weight_cod
    //                             )}</td>
    //                             <td>${submitted_by_bpf}</td>
    //                         </tr>
    //                         `;
    //           clf_pending_list_commission_form.insertAdjacentHTML(
    //             "beforeend",
    //             data_value
    //           );
    //           bpf_data_counter += 1;
    ...
    //             var waste_id_clf =
    //               clf_data_list.content[x][
    //                 findTextInArray(clf_data_list, "WASTE ID")
    //               ];
    //             if (
    //               client_id_clf == client_id_cod &&
    //               waste_id_clf == waste_id_cod
    ...
    //                             )}</td>
    //                             <td style="text-align: right; padding-right: 5px">${formatNumber(
    //                               parseFloat(weight_cod) *
    //                                 parseFloat(commission_price_clf)
    //                             )}</td>
    ...
    //       }
    //       ctf_form_no_container_commission_form.innerText =
    //         ctf_form_no_commission_form.value;
    ...
    function monthNameToNumber(monthName) {
      ...
      const monthIndex = months.findIndex(
        (month) => month === monthName.toUpperCase()
      );
      return monthIndex;
    }
