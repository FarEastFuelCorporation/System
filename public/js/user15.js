document.addEventListener("DOMContentLoaded", async function () {
  try {
    ...
    const tbf_response_promise = fetch(
      "https://script.google.com/macros/s/AKfycbx5g9gBi60zlkrI-GqRf7HxnVyF8ePvh1vg7ukI5ta3Kl6kQl3u54ygzuHK7irm6yt1/exec"
    );
    const tlf_response_promise = fetch(
      "https://script.google.com/macros/s/AKfycbxhS8uhT-WE1BMoyk6colLF4ToDA1WN6fj7Op4Jt2PPIZW_laVvgWzN3cP1gIKMUYI7Hg/exec"
    );
    const ftf_response_promise = fetch(
      "https://script.google.com/macros/s/AKfycby_KPLGaMLvzccQFKwKCZVrI1mkOzVDh7cySezC_OiZW6YVOR4mC5XkNbwllOe_Ua6TGA/exec"
    );
    const frf_response_promise = fetch(
      "https://script.google.com/macros/s/AKfycbz1BRgSEEjm-WwwEViGKbtqG_HodoU2O-Bw1DMySKnOytZJVtq3XmfOc4mbVhnc7Rke/exec"
    );
    ...
    const pof_response_promise = fetch(
      "https://script.google.com/macros/s/AKfycbzqVTd17rQV4G2nAWPDBoc2RNftAq92WL5DRsayPw98taWmzUIvhX9ppc27euLXjUe7PA/exec"
    );
    ...
    const ap_accounting_dashboard = document.querySelector(
      "#ap_accounting_dashboard"
    );

    const source_of_fund_ap_accounting =
      ap_accounting_dashboard.querySelector("#source_of_fund");
    const moldex_fund_ap_accounting =
      ap_accounting_dashboard.querySelector("#moldex_fund");
    const trucking_fund_ap_accounting =
      ap_accounting_dashboard.querySelector("#trucking_fund");
    const hauling_fund_ap_accounting =
      ap_accounting_dashboard.querySelector("#hauling_fund");
    const diesel_fund_ap_accounting =
      ap_accounting_dashboard.querySelector("#diesel_fund");
    const gasoline_fund_ap_accounting =
      ap_accounting_dashboard.querySelector("#gasoline_fund");
    const sir_ruels_fund_ap_accounting =
      ap_accounting_dashboard.querySelector("#sir_ruels_fund");
    const scrap_sales_ap_accounting =
      ap_accounting_dashboard.querySelector("#scrap_sales");
    const mold_runner_sales_ap_accounting =
      ap_accounting_dashboard.querySelector("#mold_runner_sales");
    const truck_scale_collection_ap_accounting =
      ap_accounting_dashboard.querySelector("#truck_scale_collection");
    const house_collection_ap_accounting =
      ap_accounting_dashboard.querySelector("#house_collection");
    const representation_fund_ap_accounting =
      ap_accounting_dashboard.querySelector("#representation_fund");
    const purchase_request_fund_ap_accounting =
      ap_accounting_dashboard.querySelector("#purchase_request_fund");
    const pr_payable_ap_accounting =
      ap_accounting_dashboard.querySelector("#pr_payable");
    const approved_purchased_request_list_container_purchasing =
      ap_accounting_dashboard.querySelector("#approved_purchased_request_list");
    const history_list_container_purchasing =
      ap_accounting_dashboard.querySelector("#purchase_request_history");
    const pcv_no_pr_input_purchasing =
      ap_accounting_dashboard.querySelector("#pcv_no_pr_input");
    ...
    function updateAmount() {
      ...
      for (let i = 1; i < ftf_data_list.content.length; i++) {
        // fund_source
        if (
          ftf_data_list.content[i][
            findTextInArray(ftf_data_list, "FUND SOURCE")
          ] == "SOURCE OF FUND"
        ) {
          source_of_fund -=
            ftf_data_list.content[i][findTextInArray(ftf_data_list, "AMOUNT")];
        } else if (
          ftf_data_list.content[i][
            findTextInArray(ftf_data_list, "FUND SOURCE")
          ] == "MOLDEX FUND"
          ...
          ftf_data_list.content[i][
            findTextInArray(ftf_data_list, "FUND SOURCE")
          ] == "TRUCK SCALE COLLECTION"
        ) {
          truck_scale_collection -=
            ftf_data_list.content[i][findTextInArray(ftf_data_list, "AMOUNT")];
        } else if (
          ftf_data_list.content[i][
            findTextInArray(ftf_data_list, "FUND SOURCE")
        ...
        if (
          ftf_data_list.content[i][
            findTextInArray(ftf_data_list, "FUND ALLOCATION")
          ] == "SOURCE OF FUND"
        ) {
          source_of_fund +=
            ftf_data_list.content[i][findTextInArray(ftf_data_list, "AMOUNT")];
    ...
    truck_scale_collection_ap_accounting.innerText = formatNumber(
      truck_scale_collection
    );
    house_collection_ap_accounting.innerText = formatNumber(house_collection);
    representation_fund_ap_accounting.innerText =
      formatNumber(representation_fund);
    purchase_request_fund_ap_accounting.innerText = formatNumber(
      purchase_request_fund
    );
    pr_payable_ap_accounting.innerText = formatNumber(pr_payable);

    const pending_list_container_ap_accounting =
      ap_accounting_dashboard.querySelector("#pending_list");
    const history_list_container_ap_accounting =
      document.querySelector("#history_list");
    const liquidation_history_list_container_ap_accounting =
      document.querySelector("#liquidation_history_list");
    ...
    for (let i = 1; i < ltf_data_list.content.length; i++) {
      if (
        !ltf_transaction_ap_accounting.includes(ltf_data_list.content[i][0])
      ) {
    ...
    for (let i = 1; i < tbf_data_list.content.length; i++) {
      if (
        !ltf_tbf_transaction_ap_accounting.includes(tbf_data_list.content[i][2])
      ) {
    ...
    for (let i = 1; i < tlf_data_list.content.length; i++) {
      if (
        !ltf_tlf_transaction_ap_accounting.includes(tlf_data_list.content[i][3])
      ) {
    ...
    for (let i = 1; i < tbf_data_list.content.length; i++) {
      if (
        !tbf_transaction_ap_accounting.includes(tbf_data_list.content[i][0])
      ) {
    ...
    for (let i = 1; i < tlf_data_list.content.length; i++) {
      if (
        !tbf_tlf_transaction_ap_accounting.includes(tlf_data_list.content[i][1])
      ) {
    ...
    const pending_list_ltf_ap_accounting = ltf_transaction_ap_accounting.filter(
      (element) => !ltf_tbf_transaction_ap_accounting.includes(element)
    );
    const done_list_ltf_ap_accounting = ltf_transaction_ap_accounting.filter(
      (element) => ltf_tbf_transaction_ap_accounting.includes(element)
    );
    const pending_list_tbf_ap_accounting = tbf_transaction_ap_accounting.filter(
      (element) => !tbf_tlf_transaction_ap_accounting.includes(element)
    );
    ...
    for (let x = ltf_data_list.content.length - 1; x >= 1; x--) {
      for (let y = 0; y < pending_list_ltf_ap_accounting.length; y++) {
        if (pending_list_ltf_ap_accounting[y] == ltf_data_list.content[x][0]) {
          if (
            ltf_data_list.content[x][6] == "10 WHEELER WING VAN" &&
            ltf_data_list.content[x][7] !== "NAY2264"
          ) {
            var client_name = "";
            for (let c = 1; c < client_data_list.content.length; c++) {
              if (
                ltf_data_list.content[x][2] == client_data_list.content[c][0]
              ) {
            ...
            for (let c = 1; c < type_of_waste_data.content.length; c++) {
              if (
                ltf_data_list.content[x][3] == type_of_waste_data.content[c][0]
              ) {
            ...
            data_value += `
                        <tr>
                            ...
                            <td>${date_decoder(
                              ltf_data_list.content[x][4]
                            )}<br>${time_decoder2(
              ltf_data_list.content[x][5]
            )}</td>
                            <td>${client_name}</td>
                            <td>${waste_name}</td>
    ...
    for (let i = 1; i < prf_data_list.content.length; i++) {
      if (
        prf_data_list.content[i][findTextInArray(prf_data_list, "STATUS")] ==
        "PENDING"
      ) {
        pending.push(
          prf_data_list.content[i][findTextInArray(prf_data_list, "ITM #")]
        );
      }
      if (
        prf_data_list.content[i][findTextInArray(prf_data_list, "STATUS")] ==
    ...
    for (let i = 1; i < pof_data_list.content.length; i++) {
      if (
        !prf_pr_transaction.includes(
          pof_data_list.content[i][findTextInArray(prf_data_list, "ITM #")]
        )
      ) {
        prf_pr_transaction.push(
          pof_data_list.content[i][findTextInArray(prf_data_list, "ITM #")]
        );
      }
    }
    ...
    for (let x = 1; x < prf_data_list.content.length; x++) {
      for (let y = 0; y < approved.length; y++) {
        if (
          ...
          unit =
            prf_data_list.content[x][findTextInArray(prf_data_list, "UNIT")];
          item =
            prf_data_list.content[x][findTextInArray(prf_data_list, "ITEM")];
          details =
            prf_data_list.content[x][findTextInArray(prf_data_list, "DETAILS")];
          remarks =
            prf_data_list.content[x][findTextInArray(prf_data_list, "REMARKS")];
          department =
            prf_data_list.content[x][
              findTextInArray(prf_data_list, "DEPARTMENT")
            ];
          status =
          ...
          for (let z = 1; z < pof_data_list.content.length; z++) {
            if (
              pr_data ==
              pof_data_list.content[z][findTextInArray(pof_data_list, "ITM #")]
            ) {
              amount =
                pof_data_list.content[z][
                  findTextInArray(pof_data_list, "AMOUNT")
                ];
              pr_no =
                pof_data_list.content[z][
                  findTextInArray(pof_data_list, "PRF #")
                ];
            }
          }
          button = `
          ...
          approved_data_value += `
                    <tr>
                        ...
                        <td>${pr_no}</td>
                        <td>${pr_data}</td>
                        <td>${date_decoder(date_time)} /<br> ${time_decoder(
            date_time
          )}</td>
                        <td>${quantity}</td>
                        <td>${unit}</td>
    ...
    approved_purchased_request_list_container_purchasing.innerHTML =
      approved_data_value;
    ...
    history_list_container_purchasing.insertAdjacentHTML(
      "beforeend",
      released_data_value
    );
    history_list_container_purchasing.insertAdjacentHTML(
        ...
        if (done_list_ltf_ap_accounting[y] == ltf_data_list.content[x][0]) {
          if (ltf_data_list.content[x][6] == "10 WHEELER WING VAN") {
            ...
            for (let z = 1; z < tbf_data_list.content.length; z++) {
              if (
                done_list_ltf_ap_accounting[y] == tbf_data_list.content[z][2]
              ) {
            ...
            for (let a = 0; a < ltf_tlf_transaction_ap_accounting.length; a++) {
              for (let z = 1; z < tlf_data_list.content.length; z++) {
                if (
                  done_list_ltf_ap_accounting[y] == tlf_data_list.content[z][3]
                ) {
                  if (tlf_data_list.content[z][4] == "CASH RETURN") {
                    actual_budget = (
                      tbf_budget - tlf_data_list.content[z][5]
                    ).toFixed(2);
                    remarks = "LIQUIDATED";
                  }
                }
    ...
    for (let x = tlf_data_list.content.length - 1; x >= 1; x--) {
      data_value += `
            <tr>
                ...
                <td>${date_decoder(
                  tlf_data_list.content[x][7]
                )}<br>${time_decoder2(tlf_data_list.content[x][7])}</td>
    ...
    const budget_form_button_ap_accounting =
      trip_forms_ap_accounting.querySelector("#budget_form_button");
    const liquidation_form_button_ap_accounting =
      trip_forms_ap_accounting.querySelector("#liquidation_form_button");
    const payroll_button_ap_accounting =
      trip_forms_ap_accounting.querySelector("#payroll_button");
    const payroll_tab_ap_accounting =
      trip_forms_ap_accounting.querySelector("#payroll_tab");
    const payslip_container_ap_accounting =
      trip_forms_ap_accounting.querySelector("#payslip_container");
    const search_year_ap_accounting =
      trip_forms_ap_accounting.querySelector("#search_year");
    const search_week_number_ap_accounting =
      trip_forms_ap_accounting.querySelector("#search_week_number");
    const pay_date_ap_accounting =
      trip_forms_ap_accounting.querySelector("#pay_date");
    const generate_payslip_button_ap_accounting =
      trip_forms_ap_accounting.querySelector("#generate_payslip_button");

    const budget_form_ap_accounting =
      trip_forms_ap_accounting.querySelector("#budget_form");
    const search_ltf_form_no_ap_accounting =
      budget_form_ap_accounting.querySelector("#search_ltf_form_no");
    const search_ltf_result_container_ap_accounting =
      budget_form_ap_accounting.querySelector("#search_ltf_result");
    const search_ltf_form_no_button_ap_accounting =
      budget_form_ap_accounting.querySelector("#search_ltf_form_no_button");
    const reset_ap_accounting =
      budget_form_ap_accounting.querySelector("#reset");
    const budget_form_inputs_ap_accounting =
      budget_form_ap_accounting.querySelector("#budget_form_inputs");
    const budget_form_buttons_ap_accounting =
      budget_form_ap_accounting.querySelector("#budget_form_buttons");
    const available_funds_ap_accounting =
      budget_form_ap_accounting.querySelector("#available_funds");

    const liquidation_form_ap_accounting =
      trip_forms_ap_accounting.querySelector("#liquidation_form");
    const search_tbf_form_no_ap_accounting =
      liquidation_form_ap_accounting.querySelector("#search_tbf_form_no");
    const search_tbf_result_container_ap_accounting =
      liquidation_form_ap_accounting.querySelector("#search_tbf_result");
    const ltf_no_ap_accounting =
      liquidation_form_ap_accounting.querySelector("#ltf_no");
    const pcv_no_ap_accounting =
      liquidation_form_ap_accounting.querySelector("#pcv_no");
    const search_tbf_form_no_button_ap_accounting =
      liquidation_form_ap_accounting.querySelector(
        "#search_tbf_form_no_button"
      );
    const clear_ltf_form_no_button_ap_accounting =
      liquidation_form_ap_accounting.querySelector("#clear_ltf_form_no_button");
    const liquidation_form_inputs_ap_accounting =
      liquidation_form_ap_accounting.querySelector("#liquidation_form_inputs");
    const liquidation_form_buttons_ap_accounting =
      liquidation_form_ap_accounting.querySelector("#liquidation_form_buttons");
    const budget_ap_accounting =
      liquidation_form_ap_accounting.querySelector("#budget");
    const liquidation_list_ap_accounting =
      liquidation_form_ap_accounting.querySelector("#liquidation_list");
    const diesel_gasoline_expense_container_ap_accounting =
      liquidation_form_ap_accounting.querySelector(
        "#diesel_gasoline_expense_container"
      );
    const truck_helper_count_ap_accounting =
      liquidation_form_ap_accounting.querySelector("#truck_helper_count");
    const cash_return_container_ap_accounting =
      liquidation_form_ap_accounting.querySelector("#cash_return_container");
    const add_item_button_ap_accounting =
      liquidation_form_ap_accounting.querySelector("#add_item_button");
    const remove_item_button_ap_accounting =
      liquidation_form_ap_accounting.querySelector("#remove_item_button");
    const particular_counter_ap_accounting =
      liquidation_form_ap_accounting.querySelector("#particular_counter");
    ...
    // FORM GENERATOR
    // tbf_data_list
    const tbf_no_ap_accounting =
      budget_form_ap_accounting.querySelector("#tbf_no");
    var last_row = tbf_data_list.content.length - 1;
    var data_info =
      tbf_data_list.content[last_row][findTextInArray(tbf_data_list, "TBF #")];
    ...
    search_ltf_form_no_button_ap_accounting.addEventListener("click", () => {
      ...
      for (b = 0; b < pending_list_ltf_ap_accounting.length; b++) {
        if (
          search_ltf_form_no_ap_accounting.value ==
          pending_list_ltf_ap_accounting[b]
        ) {
          for (a = 0; a < ltf_data_list.content.length; a++) {
            if (
              search_ltf_form_no_ap_accounting.value ==
              ltf_data_list.content[a][0]
            ) {
              for (let x = 1; x < employee_data_list.content.length; x++) {
                if (
                  employee_data_list.content[x][0] ==
                  ltf_data_list.content[a][8]
                ) {
              ...
              for (let c = 1; c < client_data_list.content.length; c++) {
                if (
                  ltf_data_list.content[a][2] == client_data_list.content[c][0]
                ) {
                  client_name = client_data_list.content[c][1];
                }
              }
              ...
              for (let c = 1; c < type_of_waste_data.content.length; c++) {
                if (
                  ltf_data_list.content[a][3] ==
                  type_of_waste_data.content[c][0]
                ) {
                  waste_name = type_of_waste_data.content[c][2];
                }
              }
              data_value = `
                            ...
                            HAULING DATE: ${date_decoder(
                              ltf_data_list.content[a][4]
                            )}<br>
                            HAULING TIME: ${time_decoder(
                              ltf_data_list.content[a][5]
                            )}<br>
                            ...
                            DATE DEPARTURE: ${date_decoder(
                              ltf_data_list.content[a][10]
                            )}<br>
                            TIME DEPARTURE: ${time_decoder(
                              ltf_data_list.content[a][11]
                            )}<br>
    ...
    // FORM GENERATOR
    // tlf_data_list
    const tlf_no_ap_accounting =
      liquidation_form_ap_accounting.querySelector("#tlf_no");
    var last_row = tlf_data_list.content.length - 1;
    var data_info =
      tlf_data_list.content[last_row][findTextInArray(tlf_data_list, "TLF #")];
    var data_counter = data_info.substring(9, 13);
    var month = (new Date().getMonth() + 1).toString().padStart(2, "0");
    data_counter = (parseInt(data_counter) + 1).toString().padStart(4, "0");
    var current_year = new Date().getFullYear();
    ...
    search_tbf_form_no_button_ap_accounting.addEventListener("click", () => {
      ...
      for (b = 0; b < pending_list_tbf_ap_accounting.length; b++) {
        if (
          search_tbf_form_no_ap_accounting.value ==
          pending_list_tbf_ap_accounting[b]
        ) {
          for (let y = 1; y < tbf_data_list.content.length; y++) {
            if (
              search_tbf_form_no_ap_accounting.value ==
              tbf_data_list.content[y][0]
            ) {
          ...
          for (a = 1; a < ltf_data_list.content.length; a++) {
            for (let x = 1; x < employee_data_list.content.length; x++) {
              for (let c = 1; c < tbf_data_list.content.length; c++) {
                if (
                  search_tbf_form_no_ap_accounting.value ==
                  tbf_data_list.content[c][0]
                ) {
                  if (
                    ltf_data_list.content[a][0] == tbf_data_list.content[c][2]
                  ) {
                    var cash_advance = "";
                    if (
                      employee_data_list.content[x][0] ==
                      ...
                      cash_advance += `
                                            ...
                                            `;
                      liquidation_list_ap_accounting.insertAdjacentHTML(
                        "afterbegin",
                        cash_advance
                      );
                    }
                ...
                if (employee_data_list.content[x][0] == valueArray[c]) {
                  ...
                  cash_advance += `
                                    <div>
                                        ...
                                        <input type="text" id="cash_advance${
                                          c + 1
                                        }" name="cash_advance${
                    c + 1
                  }" class="form-control" autocomplete="off" readonly value='${truck_helper_name}'>
                                    </div>
                                    <div>
                                        <label for="cash_advance_amount${
                                          c + 1
                                            ...
                                            Amount
                                            <input type="number" id="cash_advance_amount${
                                              c + 1
                    ...
                    c + 1
                                    ...
                                    `;
                  truck_helper_count_ap_accounting.value =
                    parseFloat(truck_helper_count_ap_accounting.value) + 1;
                  diesel_gasoline_expense_container_ap_accounting.insertAdjacentHTML(
                    "beforebegin",
                    cash_advance
                  );
            ...
            for (let c = 1; c < type_of_waste_data.content.length; c++) {
              if (
                ltf_data_list.content[a][3] == type_of_waste_data.content[c][0]
              ) {
                waste_name = type_of_waste_data.content[c][2];
              }
            }
            data_value = `
                        ...
                        REMARKS: ${ltf_data_list.content[a][12]}<br>
                        SUBMITTED BY: ${ltf_data_list.content[a][13]}<br>
                        RELEASED BUDGET: ${parseFloat(released_budget).toFixed(
                          2
                        )}<br>
            ...
            const inputElements =
              liquidation_list_ap_accounting.querySelectorAll(
                'input[type="number"]'
              );
    ...
    add_item_button_ap_accounting.addEventListener("click", () => {
      particular_counter_ap_accounting.value =
        parseFloat(particular_counter_ap_accounting.value) + 1;
      var data = `
      ...
      cash_return_container_ap_accounting.insertAdjacentHTML(
        "beforebegin",
        data
      );

      // Get references to the input elements
      const inputElements = liquidation_list_ap_accounting.querySelectorAll(
        'input[type="number"]'
      );
      const totalInput = document.getElementById("liquidated_budget");
      const remaining = document.getElementById("remaining");
    ...
    remove_item_button_ap_accounting.addEventListener("click", () => {
      ...
      const lastParticularContainers = document.querySelectorAll(
        `#particular_container${particular_counter_ap_accounting.value}`
      );
    ...
    generate_payslip_button_ap_accounting.addEventListener("click", () => {
      ...
      for (let x = 1; x < ltf_data_list.content.length; x++) {
        for (let y = 1; y < tlf_data_list.content.length; y++) {
          if (tlf_data_list.content[y][3] == ltf_data_list.content[x][0]) {
            if (
              search_year_ap_accounting.value ==
                getYearFromDate(date_decoder(ltf_data_list.content[x][4])) &&
              search_week_number_ap_accounting.value ==
                getWeekNumber(date_decoder(ltf_data_list.content[x][4]))
            ) {
              if (
                !tlf_transaction_ap_accounting.includes(
                  tlf_data_list.content[y][4]
                )
              ) {
      ...
      for (let z = 0; z < tlf_transaction_ap_accounting.length; z++) {
        for (let x = 1; x < employee_data_list.content.length; x++) {
          if (
            tlf_transaction_ap_accounting[z] == employee_data_list.content[x][0]
          ) {
      ...
      for (let z = 0; z < employee_array.length; z++) {
        ...
        for (let x = 0; x < client_distance.length; x++) {
          if (parseFloat(client_distance[x]) > 50) {
            ...
            if (employee_designation[0] == "DRIVER") {
              additional_rate.push(percentage * 0.6);
            } else {
              additional_rate.push(percentage * 0.4);
            }
          } else {
            if (employee_designation[0] == "DRIVER") {
        ...
        for (let x = 0; x < additional_rate.length; x++) {
          net_pay.push(
            parseFloat(employee_rate) +
              200 +
              parseFloat(additional_rate[x]) -
              parseFloat(cash_advance[x])
          );
        }
        var total_net_pay = 0;
                        ...
                        <div class="summary">
                            ...
                            <div class="employee_details">
                                <div class="grid1">
                                    ...
                                    <div class="d-flex">
                                        <h4 class="bold">Pay Period:</h4><h4 class="ps-1 truncate_text">${date_decoder(
                                          date[0]
                                        )} - ${date_decoder(date[6])}</h4>
                                    </div>
                                    <div class="d-flex">
                                    ...
                                    <table>
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td class="truncate_text2">${
                                                  client_name[index]
                                                }</td>
                                                <td>${
                                                  client_distance[index]
                                                }</td>
                                                <td>${formatNumber(
                                                  employee_rate
                                                )}</td>
    ...
    // FORM GENERATOR
    // ftf_data_list
    const fund_transfer_form_ap_accounting = document.querySelector(
      "#fund_transfer_form"
    );
    const fund_transfer_form_button_ap_accounting =
      fund_transfer_form_ap_accounting.querySelector(
        "#fund_transfer_form_button"
      );
    const fund_transfer_form_id_ap_accounting =
      fund_transfer_form_ap_accounting.querySelector("#fund_transfer_form_id");
    const transfer_history_list_ap_accounting =
      fund_transfer_form_ap_accounting.querySelector("#transfer_history_list");
    const filter_fund_source_ap_accounting =
      fund_transfer_form_ap_accounting.querySelector("#filter_fund_source");
    const filter_tab_fund_source_ap_accounting =
      fund_transfer_form_ap_accounting.querySelector("#filter_tab_fund_source");
    const filter_fund_allocation_ap_accounting =
      fund_transfer_form_ap_accounting.querySelector("#filter_fund_allocation");
    const filter_tab_fund_allocation_ap_accounting =
      fund_transfer_form_ap_accounting.querySelector(
        "#filter_tab_fund_allocation"
      );
    const add_item_button2_ftf_ap_accounting =
      fund_transfer_form_ap_accounting.querySelector("#add_item_button2_ftf");
    const remove_item_button2_ftf_ap_accounting =
      fund_transfer_form_ap_accounting.querySelector(
        "#remove_item_button2_ftf"
      );
    const data_counter_ap_accounting =
      fund_transfer_form_ap_accounting.querySelector("#data_counter");
    const fund_transfer_form_buttons_ap_accounting =
      fund_transfer_form_ap_accounting.querySelector(
        "#fund_transfer_form_buttons"
      );

    const ftf_form_no_ap_accounting =
      fund_transfer_form_ap_accounting.querySelector("#ftf_form_no");
    var last_row = ftf_data_list.content.length - 1;
    var data_info =
      ftf_data_list.content[last_row][
        findTextInArray(ftf_data_list, "FTF # / FORM #")
      ];
    var data_counter = data_info.substring(9, 13);
    var month = (new Date().getMonth() + 1).toString().padStart(2, "0");
    data_counter = (parseInt(data_counter) + 1).toString().padStart(4, "0");
    ...
    add_item_button2_ftf_ap_accounting.addEventListener("click", () => {
      data_counter_ap_accounting.value =
        parseInt(data_counter_ap_accounting.value) + 1;
      addNewData();
    });

    remove_item_button2_ftf_ap_accounting.addEventListener("click", () => {
      const item_data = document.querySelector(
        `#data_container${data_counter_ap_accounting.value}`
      );
      item_data.remove();
      data_counter_ap_accounting.value =
        parseInt(data_counter_ap_accounting.value) - 1;
    ...
    function addNewData() {
      var data = `
            ...
            `;
      fund_transfer_form_buttons_ap_accounting.insertAdjacentHTML(
        "beforebegin",
        data
      );
      updateFunction();
    ...
    function updateFundSourceAmount(
      source,
      source2,
      source3,
      source4,
      ...
      mold_runner_sales +=
        deduction_mold_runner_sales + additional_mold_runner_sales;
      truck_scale_collection +=
        deduction_truck_scale_collection + additional_truck_scale_collection;
      house_collection +=
        deduction_house_collection + additional_house_collection;
      representation_fund +=
        deduction_representation_fund + additional_representation_fund;
      purchase_request_fund +=
        deduction_representation_fund + additional_purchase_request_fund;
    ...
    function updateFundSourceAmount2(
      source,
      source2,
      source3,
      source4,
    ...
    function updateFunction() {
      const data_ap_accounting =
        fund_transfer_form_ap_accounting.querySelectorAll("#data");
      data_ap_accounting.forEach((data, index) => {
        const fund_source_ap_accounting = data.querySelector(
          `#fund_source${index + 1}`
        );
        const fund_allocation_ap_accounting = data.querySelector(
          `#fund_allocation${index + 1}`
        );
        const fund_source_amount_ap_accounting = data.querySelector(
          `#fund_source_amount${index + 1}`
        );
        const fund_allocation_amount_ap_accounting = data.querySelector(
          `#fund_allocation_amount${index + 1}`
        );
        const fund_source_amount_container_ap_accounting = data.querySelector(
          `#fund_source_amount_container${index + 1}`
        );
        const fund_allocation_amount_container_ap_accounting =
          data.querySelector(`#fund_allocation_amount_container${index + 1}`);
        const fund_source_other_details_ap_accounting = data.querySelector(
          `#fund_source_other_details_container${index + 1}`
        );
        const fund_allocation_other_details_ap_accounting = data.querySelector(
          `#fund_allocation_other_details_container${index + 1}`
        );
        const fund_amount_ap_accounting = data.querySelector(
          `#fund_amount${index + 1}`
        );
        fund_source_ap_accounting.addEventListener("change", function () {
          updateFundSourceAmount(
            fund_source_ap_accounting,
            fund_source_amount_container_ap_accounting,
            fund_source_amount_ap_accounting,
            fund_source_other_details_ap_accounting,
            fund_amount_ap_accounting.value
          );
        });

        fund_allocation_ap_accounting.addEventListener("change", function () {
          updateFundSourceAmount(
            fund_allocation_ap_accounting,
            fund_allocation_amount_container_ap_accounting,
            fund_allocation_amount_ap_accounting,
            fund_allocation_other_details_ap_accounting,
            fund_amount_ap_accounting.value
          );
        });
        fund_amount_ap_accounting.addEventListener("change", function () {
          updateFundSourceAmount2(
            fund_source_ap_accounting,
            fund_source_amount_container_ap_accounting,
            fund_source_amount_ap_accounting,
            fund_source_other_details_ap_accounting,
    ...
    // FORM GENERATOR
    // frf_data_list
    const fund_request_form_button_ap_accounting =
      fund_transfer_form_ap_accounting.querySelector(
        "#fund_request_form_button"
      );
    const fund_request_form_id_ap_accounting =
      fund_transfer_form_ap_accounting.querySelector("#fund_request_form_id");
    const add_item_button2_ap_accounting =
      fund_request_form_id_ap_accounting.querySelector("#add_item_button");
    const remove_item_button2_ap_accounting =
      fund_request_form_id_ap_accounting.querySelector("#remove_item_button");
    const request_list_container_ap_accounting =
      fund_request_form_id_ap_accounting.querySelector(
        "#request_list_container"
      );
    const request_list_counter_ap_accounting =
      fund_request_form_id_ap_accounting.querySelector("#request_list_counter");
    const request_history_list_ap_accounting =
      fund_transfer_form_ap_accounting.querySelector("#request_history_list");

    const frf_form_no_ap_accounting =
      fund_request_form_id_ap_accounting.querySelector("#frf_form_no1");
    var last_row = frf_data_list.content.length - 1;
    var data_info =
      frf_data_list.content[last_row][findTextInArray(frf_data_list, "FRF #")];
    var data_counter = data_info.substring(9, 13);
    var month = (new Date().getMonth() + 1).toString().padStart(2, "0");
    data_counter = (parseInt(data_counter) + 1).toString().padStart(4, "0");
    var current_year = new Date().getFullYear();
    ...
    add_item_button2_ap_accounting.addEventListener("click", () => {
      ...
      data_counter_value = String(
        parseFloat(data_counter_value) +
          parseFloat(request_list_counter_ap_accounting.value)
      ).padStart(3, "0");
      request_list_counter_ap_accounting.value =
        parseFloat(request_list_counter_ap_accounting.value) + 1;
      var data = `
      ...
      request_list_container_ap_accounting.insertAdjacentHTML(
        "beforeend",
        data
      );
    });
    ...
    remove_item_button2_ap_accounting.addEventListener("click", () => {
      // Find the last added particular container
      const last_request_list = document.querySelector(
        `#request_list${request_list_counter_ap_accounting.value}`
      );
    ...
    function generateData(fund_source = "ALL", fund_allocation = "ALL") {
      ...
      for (let x = ftf_data_list.content.length - 1; x >= 1; x--) {
        if (
          ftf_data_list.content[x][findTextInArray(ftf_data_list, "DATE")] == ""
        ) {
          transaction_date =
            ftf_data_list.content[x][
              findTextInArray(ftf_data_list, "CREATED AT")
            ];
        } else {
          transaction_date =
            ftf_data_list.content[x][findTextInArray(ftf_data_list, "DATE")];
        ...
        if (fund_source == "ALL" && fund_allocation == "ALL") {
          ...
          data += `
                    <tr>
                        ...
                        <td>${formatNumber(
                          ftf_data_list.content[x][
                            findTextInArray(ftf_data_list, "AMOUNT")
                          ]
                        )}</td>
                        <td>${
                          ftf_data_list.content[x][
                            findTextInArray(ftf_data_list, "SUBMITTED BY")
                          ]
                        }</td>
                        <td>${
                          ftf_data_list.content[x][
                            findTextInArray(ftf_data_list, "REMARKS")
                          ]
                        }</td>
                    </tr>
                    `;
    ...
    filter_tab_fund_source_ap_accounting.addEventListener("change", () => {
      var filter_fund_source =
        filter_tab_fund_source_ap_accounting.value || "ALL";
      var filter_fund_allocation =
        filter_tab_fund_allocation_ap_accounting.value || "ALL";
    ...
    for (let x = frf_data_list.content.length - 1; x >= 1; x--) {
      ...
      if (frf_data_list.content[x][9] !== "") {
        ...
        duration = calculateTravelTime(
          date_decoder(frf_data_list.content[x][8]),
          time_decoder2(frf_data_list.content[x][8]),
          date_decoder(frf_data_list.content[x][9]),
          time_decoder2(frf_data_list.content[x][9])
        );
      }
      ...
      data += `
            <tr>
                ...
                <td>${date_decoder(
                  frf_data_list.content[x][8]
                )}<br>${time_decoder2(frf_data_list.content[x][8])}</td>
