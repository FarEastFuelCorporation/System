document.addEventListener('DOMContentLoaded', async function() {
    try {
        ...
        const pr_response_promise = fetch('https://script.google.com/macros/s/AKfycbxihvgnFRQOVXRo25fVK5vLW0pJ8eUtyh04Tl9DDHH6wXQPvmSCq0yCwYYFAbDcx4OTVQ/exec');
        ...
        const pr_data_list  = await pr_response.json();
        ...
        let orf_transaction_treatment = []; // Variable containing existing elements
        let orf_pr_transaction_treatment = []; // Variable containing existing elements
        
        for (let i = 1; i < orf_data_list.content.length; i++) {
            if (!orf_transaction_treatment.includes(orf_data_list.content[i][1])) {
                orf_transaction_treatment.push(orf_data_list.content[i][1]);
            }
        }

        for (let i = 1; i < pr_data_list.content.length; i++) {
            if (!orf_pr_transaction_treatment.includes(pr_data_list.content[i][2])) {
                orf_pr_transaction_treatment.push(pr_data_list.content[i][2]);
            }
        }

        // Get elements from sf_transaction not included in sf_tpf_transaction
        const pending_list_purchasing = orf_transaction_treatment.filter((element) => !orf_pr_transaction_treatment.includes(element));

        ordered_transactions_purchasing.innerText = orf_transaction_treatment.length;
        ...
        for(let x = 1; orf_data_list.content.length; x++){
